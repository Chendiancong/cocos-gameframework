import { gfx, UIRenderer, Event, Vec2, Color, isValid, Node, game, director, macro, Layers, view, Font, resources, Vec3, Rect, UITransform, UIOpacity, Component, Graphics, misc, Sprite, Size, screen, Asset, Texture2D, warn, AudioClip, BitmapFont, SpriteFrame, sp, dragonBones, assetManager, error, ImageAsset, BufferAsset, AssetManager, path, Label, SpriteAtlas, RichText, sys, EventMouse, EventTarget, Mask, math, rect, View, AudioSourceComponent, EditBox, Overflow, log, Director, Prefab, instantiate, Widget } from 'cc';
...
var PackageLoadType;
(function (PackageLoadType) {
    PackageLoadType[PackageLoadType["None"] = 0] = "None";
    PackageLoadType[PackageLoadType["Image"] = 1] = "Image";
    PackageLoadType[PackageLoadType["Atlas"] = 2] = "Atlas";
    PackageLoadType[PackageLoadType["Sound"] = 4] = "Sound";
    PackageLoadType[PackageLoadType["Default"] = 2] = "Default";
    PackageLoadType[PackageLoadType["All"] = 7] = "All";
})(PackageLoadType || (PackageLoadType = {}));
var PackageItemState;
(function (PackageItemState) {
    PackageItemState[PackageItemState["Init"] = 0] = "Init";
    PackageItemState[PackageItemState["Loading"] = 1] = "Loading";
    PackageItemState[PackageItemState["Loaded"] = 2] = "Loaded";
    PackageItemState[PackageItemState["Decoding"] = 4] = "Decoding";
    PackageItemState[PackageItemState["Decoded"] = 8] = "Decoded";
    PackageItemState[PackageItemState["LoadedOrLoading"] = 3] = "LoadedOrLoading";
})(PackageItemState || (PackageItemState = {}));
var UIPackageLoadingMode;
(function (UIPackageLoadingMode) {
    /** 默认（同步加载） */
    UIPackageLoadingMode[UIPackageLoadingMode["Normal"] = 0] = "Normal";
    /** 异步加载 */
    UIPackageLoadingMode[UIPackageLoadingMode["Async"] = 1] = "Async";
    /** 按需加载 */
    UIPackageLoadingMode[UIPackageLoadingMode["Auto"] = 2] = "Auto";
})(UIPackageLoadingMode || (UIPackageLoadingMode = {}));
var ObjectType;
(function (ObjectType) {
    ...
    ////
    ObjectType[ObjectType["MovieClip2D"] = 19] = "MovieClip2D";
    ObjectType[ObjectType["Spine2D"] = 20] = "Spine2D";
...
(function (ProgressTitleType) {
    ...
    ProgressTitleType[ProgressTitleType["MaxTxt"] = 4] = "MaxTxt";
    ProgressTitleType[ProgressTitleType["None"] = 5] = "None";
...
class BlendModeUtils {
    static apply(node, blendMode) {
        ...
        renderers.forEach(element => {
            element._srcBlendFactor = f[0];
            element._dstBlendFactor = f[1];
        });
    }
...
class MyEvent extends Event {
    ...
    upCaptureTouch() {
        let obj = GObject.cast(this.currentTarget);
        if (obj)
            this._processor.removeTouchMonitor(obj);
    }
}
MyEvent.TOUCH_BEGIN = "fui_touch_begin";
MyEvent.TOUCH_MOVE = "fui_touch_move";
MyEvent.TOUCH_END = "fui_touch_end";
MyEvent.CLICK = "fui_click";
MyEvent.ROLL_OVER = "fui_roll_over";
MyEvent.ROLL_OUT = "fui_roll_out";
MyEvent.MOUSE_WHEEL = "fui_mouse_wheel";
MyEvent.DISPLAY = "fui_display";
MyEvent.UNDISPLAY = "fui_undisplay";
MyEvent.GEAR_STOP = "fui_gear_stop";
MyEvent.LINK = "fui_text_link";
MyEvent.Submit = "editing-return";
MyEvent.TEXT_CHANGE = "text-changed";
MyEvent.STATUS_CHANGED = "fui_status_changed";
MyEvent.XY_CHANGED = "fui_xy_changed";
MyEvent.SIZE_CHANGED = "fui_size_changed";
MyEvent.SIZE_DELAY_CHANGE = "fui_size_delay_change";
MyEvent.COLLECTION_CHANGED = "fui_collection_changed";
MyEvent.DRAG_START = "fui_drag_start";
MyEvent.DRAG_MOVE = "fui_drag_move";
MyEvent.DRAG_END = "fui_drag_end";
MyEvent.DROP = "fui_drop";
MyEvent.SCROLL = "fui_scroll";
MyEvent.SCROLL_END = "fui_scroll_end";
MyEvent.PULL_DOWN_RELEASE = "fui_pull_down_release";
MyEvent.PULL_UP_RELEASE = "fui_pull_up_release";
MyEvent.SELECT_ITEM = "fui_select_item";
MyEvent.CLICK_ITEM = "fui_click_item";
MyEvent.LOADED = "fui_loaded";
MyEvent.COMPLETE = "fui_complete";
MyEvent.WINTWEENEND = "tweenend";
...
class GearColor extends GearBase {
    ...
    selfSetup(values) {
        values.split("|").forEach(((str, index) => {
            var gv = this._storage[index];
            if (!gv)
                gv = { color: new Color(), strokeColor: new Color() };
            if (gv) {
                let temp = str.split(",");
                gv.color.fromHEX(temp[0]);
                gv.strokeColor.fromHEX(temp[1]);
            }
        }));
        this.apply();
    }
...
function evaluateEase(easeType, time, duration, overshootOrAmplitude, period) {
    switch (easeType) {
        ...
        case EaseType.Custom:
            if (time == duration)
                return 1;
            return (-Math.pow(3, -10 * time / duration) + 1);
        default:
            return -(time /= duration) * (time - 2);
    }
}
...
class AdaptEvent extends MyEvent {
    asDelayAdaptSizeEvt() {
        this.type = AdaptEvent.DELAY_ADAPT_SIZE;
        return this;
    }
    setSizeChange(curWidth, curHeight, originWidth, originHeight) {
...
AdaptEvent.DELAY_ADAPT_SIZE = "delay_adapt_size";
const adaptEvent = new AdaptEvent(AdaptEvent.DELAY_ADAPT_SIZE, false);
...
class RelationItem {
    ...
    getHasDef(relationType) {
        if (relationType == RelationType.Size) {
            return this.getHasDef(RelationType.Width) && this.getHasDef(RelationType.Height);
        }
        var length = this._defs.length;
        for (var i = 0; i < length; i++) {
            var info = this._defs[i];
            if (info.type == relationType)
                return true;
        }
        return false;
    }
    ...
    addRefTarget(target) {
        if (target != this._owner.parent)
            target.on(MyEvent.XY_CHANGED, this.__targetXYChanged, this);
        target.on(MyEvent.SIZE_CHANGED, this.__targetSizeChanged, this);
        target.on(MyEvent.SIZE_DELAY_CHANGE, this.__targetSizeWillChange, this);
        target.on(AdaptEvent.DELAY_ADAPT_SIZE, this.__onDelayAdaptSize, this);
    ...
    releaseRefTarget(target) {
        if (!target.node)
            return;
        target.off(MyEvent.XY_CHANGED, this.__targetXYChanged, this);
        target.off(MyEvent.SIZE_CHANGED, this.__targetSizeChanged, this);
        target.off(MyEvent.SIZE_DELAY_CHANGE, this.__targetSizeWillChange, this);
        target.off(AdaptEvent.DELAY_ADAPT_SIZE, this.__onDelayAdaptSize, this);
    }
    ...
    __onDelayAdaptSize(evt) {
        this._targetWidth = evt.originWidth;
        this._targetHeight = evt.originHeight;
        this.__targetSizeChanged(evt);
    }
}
class RelationDef {
    constructor() {
...
class Relations {
    ...
    getHasDef(relationType) {
        return !!this._items.find(item => {
            if (item.getHasDef(relationType)) {
                return true;
            }
        });
    }
...
class UIConfig {
    constructor() { }
    static get designWidth() { return view.getDesignResolutionSize().width; }
    static get designHeight() { return view.getDesignResolutionSize().height; }
    static listSetDataCheck(list, doFinish, doChildFinish) { }
}
UIConfig.residentPkgKeys = ["common", "main", "icon"];
//Default font name
...
UIConfig.packageConfig = Object.create(null);
UIConfig.defaultTaskId = 100;
UIConfig.textInputDelayInitSec = undefined;
UIConfig.listSetDataCheckFlag = false;
let _fontRegistry = {};
...
function checkIsResidentPkg(pkgName) {
    if (!pkgName)
        return false;
...
function inerlistRenderHelper(list, datas) {
    return __awaiter$3(this, void 0, void 0, function* () {
        try {
            ...
            const doSth = () => __awaiter$3(this, void 0, void 0, function* () {
                let index = 0;
                let viewCnt = calculateViewCount(list); // 优化的计算视图数量的方法
                let dataProvider = list.getOrSetDataProvider();
                if (list.virtualItemSize && viewCnt <= 0) {
                    throw new Error("Invalid view count. Ensure list.virtualItemSize, list.viewWidth, and list.viewHeight are correctly set.");
                }
                const mydataProvider = yield dataProvider;
                const processData = () => {
                    if (!list.node.isValid) {
                        _resolve(list);
                        list._partner.unschedule(processData);
                        return;
                    }
                    const sizeLen = Math.min((list.virtualItemSize ? viewCnt : datas.length), datas.length);
                    if (index >= sizeLen) {
                        _resolve(list);
                        list._partner.unschedule(processData);
                        let left = datas.length - sizeLen;
                        if (left > 0) {
                            while (left > 0) {
                                addItem(datas[index]);
                                index++;
                ...
                const addItem = (data) => {
                    mydataProvider.addItem(data);
                };
                list._partner.schedule(processData, 0);
            });
            doSth.apply(list);
            return p;
        }
        catch (error) {
            console.error("Failed to render list helper:", error);
            throw error; // Re-throwing the error to make sure the caller can handle it if needed
        }
    });
}
function calculateViewCount(list) {
    if (!list.virtualItemSize || list.viewWidth <= 0 || list.viewHeight <= 0) {
        return -1;
    }
    return Math.ceil(list.viewWidth / (list.virtualItemSize.width + list["_columnGap"])) * Math.ceil(list.viewHeight / (list.virtualItemSize.height + list["_lineGap"]));
}
...
class GObject {
    constructor() {
        ...
        this._isFirstTimeEnabled = false;
        this._objectStates = 0 /* GObjectStates.Init */;
        this.sourceWidth = 0;
        this.sourceHeight = 0;
        this.initWidth = 0;
        this.initHeight = 0;
        ...
        this._taskId = UIConfig.defaultTaskId;
        this._name = "";
    ...
    get guide_key() {
        return this._guide_key;
    }
    set guide_key(value) {
        this._guide_key = value;
    }
    get x() {
        return this._x;
    }
    ...
    setPosition(xv, yv) {
        if (this._x != xv || this._y != yv) {
            ...
            if (this._parent && !("setVirtual" in this._parent) /*not list*/) {
                ...
                this._node.emit(MyEvent.XY_CHANGED, this);
            }
    ...
    set ccRenderer(val) {
        this.setCCRenderer(val);
    }
    addRenderer(val, params) {
        return this._internalAddRenderer(val, params, true);
    }
    removeRenderer(val) {
        this._extraRenderers.splice(this._extraRenderers.indexOf(val), 1);
    }
    _getOrAddComponent(val) {
        let comp = this._node.getComponent(val);
        if (!comp)
            comp = this._node.addComponent(val);
        return comp;
    }
    _internalAddRenderer(val, params, isExtra) {
        var ccRender;
        ccRender = val && this._getOrAddComponent(val);
        if (!!params)
            Object.assign(ccRender, params);
        if (!!ccRender && typeof ccRender.initProp == "function")
            ccRender.initProp(ccRender);
        if (isExtra)
            this._extraRenderers.push(ccRender);
        return ccRender;
    }
    get ccRender() {
        return this._ccRender;
    }
    get index() {
        if (this._parent) {
    ...
    get itemIndex() {
        let res = this.index;
        if (!!this._parent["childIndexToItemIndex"]) {
            res = this._parent["childIndexToItemIndex"](res);
        }
        return res;
    }
    center(restraint) {
    ...
    get isFirstTimeVisible() {
        return !!(this._objectStates & 2 /* GObjectStates.FirstTimeVisible */);
    }
    setSize(wv, hv, ignorePivot) {
        if (this._rawWidth != wv || this._rawHeight != hv) {
            ...
            if (!!this._node)
                this._node.emit(MyEvent.SIZE_CHANGED, this);
        }
    }
    makeFullScreen() {
        this.setSize(Decls$1.GRoot.inst.width, Decls$1.GRoot.inst.height);
    }
    makeFullContainer(target) {
    ...
    set tooltips(value) {
        if (this._tooltips) {
            this._node.off(MyEvent.ROLL_OVER, this.onRollOver, this);
            this._node.off(MyEvent.ROLL_OUT, this.onRollOut, this);
        }
        this._tooltips = value;
        if (this._tooltips) {
            this._node.on(MyEvent.ROLL_OVER, this.onRollOver, this);
            this._node.on(MyEvent.ROLL_OUT, this.onRollOut, this);
        }
    }
    get blendMode() {
    ...
    closeTask() {
        if (this._taskMaintainer) {
            if (this._taskId !== UIConfig.defaultTaskId)
                closeTask(this._taskId, true);
        }
        this._taskMaintainer = null;
    }
    onEnable() {
    }
    onDisable() {
    ...
    firstTimeVisible() {
        this._objectStates |= 2 /* GObjectStates.FirstTimeVisible */;
        this.onFirstTimeVisible();
    }
    onFirstTimeVisible() { }
    onConstruct() {
        this._objectStates |= 1 /* GObjectStates.AfterConstruct */;
        if (this._finalVisible)
            this.firstTimeVisible();
    }
    onClick(listener, target) {
        this._node.on(MyEvent.CLICK, listener, target);
    }
    onceClick(listener, target) {
        this._node.once(MyEvent.CLICK, listener, target);
    }
    offClick(listener, target) {
        this._node.off(MyEvent.CLICK, listener, target);
    }
    clearClick() {
        this._node.off(MyEvent.CLICK);
    }
    hasClickListener() {
        return this._node.hasEventListener(MyEvent.CLICK);
    }
    on(type, listener, target) {
        if (type == MyEvent.DISPLAY || type == MyEvent.UNDISPLAY)
    ...
    localToGlobal(ax, ay, result) {
        ...
        s_vec3$1.x = ax;
        s_vec3$1.y = -ay;
        let offY = GObject.draggingOffSet || 0;
        if (!this._pivotAsAnchor) {
        ...
        const realHeight = Decls$1.GRoot.inst.height;
        s_vec3$1.y = realHeight - ((realHeight - UIConfig.designHeight) >> 1) - s_vec3$1.y;
        result = result || new Vec2();
        result.x = s_vec3$1.x;
        result.y = s_vec3$1.y - offY;
        return result;
    }
    ...
    handleVisibleChanged() {
        ...
        if (this._node.active) {
            const objStates = this._objectStates;
            const targetState = 1 /* GObjectStates.ReadyForFirstTimeVisible */;
            if ((objStates & targetState) == targetState)
                this.firstTimeVisible();
        }
        if (this instanceof GGroup)
            this.handleVisibleChanged();
    ...
    set data(val) {
        this.setObjectData(val);
    }
    get data() {
        return this._data;
    }
    beforeConstructEntry() { }
    ...
    selfSetUpGear(target, index, ctr, values) {
        if (!target || !ctr || !values)
            return;
        if (!(target.node && target.node.isValid))
            return;
        target._gears[index].controller = ctr;
        target._gears[index].selfSetup(values);
    }
    setObjectData(val) {
        this._data = val;
        if (this._node) {
            if (this._ccRender != null) {
                if (this._ccRender._data != val) {
                    this._ccRender._data = val;
                    this.emitViewChange();
                }
                this._node.emit("dataChanged", val);
            }
            if (this._extraRenderers.length > 0) {
                let isViewChange = false;
                for (let i = 0, len = this._extraRenderers.length; i < len; ++i) {
                    if (this._extraRenderers[i]._data != val) {
                        this._extraRenderers[i]._data = val;
                        isViewChange = true;
                    }
                }
                if (isViewChange)
    ...
    emitViewChange() {
        this._node.emit("viewChanged");
    }
    setCCRenderer(renderer) {
        if (this._ccRenderer === renderer) {
            return;
        }
        if (this._ccRender != void 0) {
            if (typeof this._ccRender.dispose == "function")
                this._ccRender.dispose(false);
            this._ccRender.destroy();
        }
        this._ccRenderer = renderer;
        this._ccRender = this._internalAddRenderer(renderer, null, false);
    }
    //toolTips support
    onRollOver() {
    ...
    initDrag() {
        if (this._draggable) {
            this.on(MyEvent.TOUCH_BEGIN, this.onTouchBegin_0, this);
            this.on(MyEvent.TOUCH_MOVE, this.onTouchMove_0, this);
            this.on(MyEvent.TOUCH_END, this.onTouchEnd_0, this);
        }
        else {
            this.off(MyEvent.TOUCH_BEGIN, this.onTouchBegin_0, this);
            this.off(MyEvent.TOUCH_MOVE, this.onTouchMove_0, this);
            this.off(MyEvent.TOUCH_END, this.onTouchEnd_0, this);
        }
    }
    dragBegin(touchId) {
        if (GObject.draggingObject) {
            ...
            tmp._node.emit(MyEvent.DRAG_END);
        }
    ...
    onTouchMove_0(evt) {
        ...
        if (GObject.draggingObject != this && this._draggable && this._dragTesting) {
            ...
            this._dragTesting = false;
            s_dragQuery = true;
            this._node.emit(MyEvent.DRAG_START, evt);
            if (s_dragQuery)
                this.dragBegin(evt.touchId);
        }
        if (GObject.draggingObject == this) {
            let offSetX = (_a = this.offsetGlobalX) !== null && _a !== void 0 ? _a : 0;
            let offSetY = (_b = this.offsetGlobalY) !== null && _b !== void 0 ? _b : 0;
            var xx = evt.pos.x - sGlobalDragStart.x + sGlobalRect.x + offSetX;
            var yy = evt.pos.y - sGlobalDragStart.y + sGlobalRect.y + offSetY;
            if (!this._dragPos)
                this._dragPos = new Vec2(evt.pos.x, evt.pos.y);
            this._dragPos.set(evt.pos.x, evt.pos.y);
            if (this._dragBounds) {
            ...
            s_dragging = true;
            this.parent.globalToLocal(xx, yy, s_vec2$4);
            //// this.setPosition(Math.round(pt.x), Math.round(pt.y));
            this.setPosition(Math.round(xx), Math.round(yy));
            s_dragging = false;
            this._node.emit(MyEvent.DRAG_MOVE, evt);
        }
    }
    onTouchEnd_0(evt) {
        if (GObject.draggingObject == this) {
            this._node.emit(MyEvent.DRAG_END, evt, GObject.draggingObject.data);
            GObject.draggingObject = null;
        }
    }
}
...
class GObjectPartner extends Component {
    ...
    onClickLink(evt, text) {
        this.node.emit(MyEvent.LINK, text, evt);
    }
    onEnable() {
        this.node["$gobj"].onEnable();
        if (this._emitDisplayEvents)
            this.node.emit(MyEvent.DISPLAY);
    }
    onDisable() {
        this.node["$gobj"].onDisable();
        if (this._emitDisplayEvents)
            this.node.emit(MyEvent.UNDISPLAY);
    }
    update(dt) {
...
class GGroup extends GObject {
    ...
    handleAlphaChanged() {
        ...
        for (var i = 0; i < cnt; i++) {
            ...
            if (child.group == this) {
                child.alpha = this.alpha * child.alpha;
            }
        }
    }
    handleVisibleChanged() {
...
class GGraph extends GObject {
    ...
    updateGraph() {
        ...
        ctx.fillColor.set(this._fillColor.r, this._fillColor.g, this._fillColor.b, this._fillColor.a * this._alpha);
        if (this._type == 1) {
...
class Image extends Sprite {
    constructor() {
        ...
        this._fillAmount = 0;
        this._loaded = false;
        this._dirtyState = 0 /* ImageDirtyState.None */;
    }
    get flip() {
        return this._flip;
    }
    ...
    set fillMethod(value) {
        if (this._fillMethod != value) {
            ...
            if (!this._loaded) {
                this._dirtyState |= 2 /* ImageDirtyState.FillMethod */;
                return;
            }
            this._onFillMethod();
        }
    ...
    set fillOrigin(value) {
        if (this._fillOrigin != value) {
            ...
            if (!this._loaded) {
                this._dirtyState |= 4 /* ImageDirtyState.FillOrigin */;
                return;
            }
            this._onFillOrigin();
        }
    ...
    set fillClockwise(value) {
        if (this._fillClockwise != value) {
            ...
            if (!this._loaded) {
                this._dirtyState |= 16 /* ImageDirtyState.FillClockwise */;
                return;
            }
            this._onFillClockwise();
        }
    }
    get fillAmount() {
    ...
    set fillAmount(value) {
        if (this._fillAmount != value) {
            ...
            if (!this._loaded) {
                this._dirtyState |= 8 /* ImageDirtyState.FillAmount */;
                return;
            }
    ...
    setSpriteFrame(sprite) {
        ...
        this._loaded = !!sprite;
        if (this._loaded && this._dirtyState != 0 /* ImageDirtyState.None */)
            this._whenDirtyState();
    }
    setupFill() {
    ...
    _whenDirtyState() {
        const state = this._dirtyState;
        this._dirtyState = 0 /* ImageDirtyState.None */;
        if (state & 30 /* ImageDirtyState.SpriteDirty */) {
            do {
                if (state & 2 /* ImageDirtyState.FillMethod */) {
                    this._onFillMethod();
                    break;
                }
                if (this._fillMethod)
...
class GImage extends GObject {
    ...
    set grayed(value) {
        if (this._grayed == value)
            return;
        this._content.grayscale = this._grayed = value;
    }
    ;
    constructFromResource() {
        ...
        contentItem.getAsset((err, item) => {
            if (!isValid(this._content.node))
                return;
            if (item.scale9Grid)
                this._content.type = Sprite.Type.SLICED;
            else if (item.scaleByTile)
                this._content.type = Sprite.Type.TILED;
            // this._content.spriteFrame = <SpriteFrame>item.asset;
            this._content.setSpriteFrame(item.asset);
            if (this.onReady) {
                this.onReady();
                this.onReady = null;
            }
...
class MovieClip extends Image {
    ...
    drawFrame() {
        if (this._frameCount > 0 && this._frame < this._frames.length) {
            ...
            this.setSpriteFrame(frame.texture);
        }
    }
}
...
/**
 * 加载一个资源，如果缓存中存在，则直接返回，否则进行加载。
 * bundle.load本身是会检查缓存的，但无法立即返回。
 * 所以手动查询缓存，命中时就立即返回资源，否则才走资源加载的流程。
 */
function getOrLoad(bundle, url, type, onComplete, onProgress) {
    let asset = bundle.get(url, type);
    if (isValid(asset))
        onComplete(null, asset);
    bundle.load(url, type, onProgress, onComplete);
}
...
class PackageItem {
    constructor() {
        this.width = 0;
        this.height = 0;
        this.state = PackageItemState.Init;
    }
    /**
     * @deprecated
     */
    load() {
        // return this.owner.getItemAsset(this)??null;
        return null;
    }
    /**
     * 获取资源，如果没有加载则自动加载
     */
    getAsset(onLoad) {
        if (this.state & PackageItemState.Decoded)
            onLoad(null, this);
        else
            this.loadAsset(onLoad);
    }
    /**
     * 立即获取资源，资源没有加载的话会自动加载，但不会等待资源加载成功
     */
    getAssetImmediately() {
        if (this.state & PackageItemState.Decoded)
            return this.asset;
    ...
    loadAsset(onComplete) {
        if (onComplete != void 0)
            this._addCallback(onComplete);
        if (this.state & PackageItemState.Decoded)
            this._flushLoaded(null);
        this._internalLoadAsset();
    }
    aloadAsset() {
        let p, _resolve, _reject;
        p = new Promise((resolve, reject) => {
    ...
    _onAssetLoaded(err, asset) {
        ...
        this.state = PackageItemState.Decoded;
        this.asset = asset;
        this._flushLoaded(err);
    }
    _onDragonBonesAssetLoaded(err, asset, atlasAsset) {
        if (!this.owner.isValid) {
            if (isValid(asset))
                assetManager.releaseAsset(asset);
            if (isValid(atlasAsset))
                assetManager.releaseAsset(atlasAsset);
            return;
        }
        this.asset = asset;
    ...
    _addCallback(onComplete) {
        if (this.loading == void 0)
            this.loading = [onComplete];
        else
            this.loading.push(onComplete);
    }
    _flushLoaded(err = null) {
        if (err) {
            warn(`error when load ${this.file}`);
            error(err);
        }
        const arr = this.loading;
        delete this.loading;
        if (arr) {
    ...
    _internalLoadAsset() {
        var _a;
        if (this.state & PackageItemState.LoadedOrLoading)
            return;
        this.state |= PackageItemState.Loading;
        (_a = PackageItem._itemLoaders[this.type]) === null || _a === void 0 ? void 0 : _a.call(this);
    }
    _loadedAndDecoding() {
        this.state |= PackageItemState.Loaded | PackageItemState.Decoding;
    }
}
//----------------------------------------------------------------------------
PackageItem._itemLoaders = {
    [PackageItemType.Image]: function () {
        const sprite = this.owner.sprites[this.id];
        if (sprite) {
            const atlas = sprite.atlas;
            const atlasLoaded = (err, asset) => {
                this._loadedAndDecoding();
                const atlasTexture = asset;
                if (atlasTexture) {
                    const sf = new SpriteFrame();
                    sf.texture = atlasTexture;
                    sf.rect = sprite.rect;
                    sf.rotated = sprite.rotated;
                    const offsetX = sprite.offset.x - (sprite.originalSize.width - sprite.rect.width) / 2;
                    const offsetY = -(sprite.offset.y - (sprite.originalSize.height - sprite.rect.height) / 2);
                    sf.offset = new Vec2(offsetX, offsetY);
                    ...
                    this._onAssetLoaded(err, sf);
                }
                else
                    this._onAssetLoaded(err, null);
            };
            if (atlas.state & PackageItemState.Decoded)
                atlasLoaded(null, atlas.asset);
            else
                atlas.loadAsset((err, item) => atlasLoaded(err, item.asset));
        }
    },
    [PackageItemType.Atlas]: function () {
        bundleUtils.getOrLoad(this.owner.bundle, this.file, Asset, (err, asset) => {
            this._loadedAndDecoding();
            let tex = null;
            if (!err) {
                const _asset = asset;
                ...
                warn(err);
            this._onAssetLoaded(err, tex);
        });
    },
    [PackageItemType.Sound]: function () {
        this._loadedAndDecoding();
        bundleUtils.getOrLoad(this.owner.bundle, this.file, AudioClip, (err, asset) => {
            this._loadedAndDecoding();
            if (err)
                warn(err);
            this._onAssetLoaded(err, asset);
        });
    },
    [PackageItemType.MovieClip]: function () {
        let self = this;
        const buffer = this.rawData;
        buffer.seek(0, 0);
        ...
        function onAddCnt() {
            ...
            if (frameCnt >= frameCount) {
                self.state = PackageItemState.Decoded;
                self._flushLoaded(null);
            }
            else {
                ansyCallFun();
            }
        }
        function onFrameAtlasLoaded(err, pkgItem) {
            item.width / this.rect.width;
            const sf = new SpriteFrame();
            sf.texture = pkgItem.asset;
            sf.rect = sprite.rect;
            sf.rotated = sprite.rotated;
            const offsetX = this.rect.x - (item.width - this.rect.width) / 2;
            const offsetY = -(this.rect.y - (item.height - this.rect.height) / 2);
            sf.offset = new Vec2(offsetX, offsetY);
            sf.originalSize = new Size(item.width, item.height);
            this.texture = sf;
            item.frames[frameCnt] = this;
            onAddCnt();
        }
        function ansyCallFun() {
            ...
            if (spriteId != null && (sprite = self.owner.sprites[spriteId]) != null) {
                sprite.atlas.getAsset(onFrameAtlasLoaded.bind(frame));
            }
            buffer.position = nextPos;
        }
        ansyCallFun();
    },
    [PackageItemType.Font]: function () {
        return __awaiter$2(this, void 0, void 0, function* () {
            ...
            const dict = font.fntConfig.fontDefDictionary;
            const buffer = this.rawData;
            buffer.seek(0, 0);
            ...
            let mainTexture;
            let mainSprite = this.owner.sprites[this.id];
            if (mainSprite)
                mainTexture = (yield this.aloadAsset()).asset;
            buffer.seek(0, 1);
            let bg;
            ...
            for (let i = 0; i < cnt; i++) {
                ...
                if (ttf) {
                    ...
                    let sprite = mainSprite = this.owner.sprites[img];
                    if (sprite) {
                        ...
                        //TODO:
                        if (!sprite.atlas.asset) {
                            mainTexture = (yield sprite.atlas.aloadAsset()).asset;
                        }
                        else {
            ...
            font.onLoaded();
            this._onAssetLoaded(null, font);
        });
    },
    [PackageItemType.Spine]: function () {
        bundleUtils.getOrLoad(this.owner.bundle, this.file, sp.SkeletonData, (err, asset) => {
            this._loadedAndDecoding();
            this._onAssetLoaded(err, asset);
        });
    },
    [PackageItemType.DragonBones]: function () {
        bundleUtils.getOrLoad(this.owner.bundle, this.file, dragonBones.DragonBonesAsset, (err, asset) => {
            this._loadedAndDecoding();
            if (err) {
                this._onDragonBonesAssetLoaded(err, asset, null);
                return;
            }
            let atlasFile = this.file.replace("_ske", "_tex");
            let pos = atlasFile.lastIndexOf('.');
            if (pos != -1)
                atlasFile = atlasFile.substr(0, pos + 1) + "json";
            bundleUtils.getOrLoad(this.owner.bundle, atlasFile, dragonBones.DragonBonesAtlasAsset, (err, atlasAsset) => {
                this.state |= PackageItemState.Decoded;
                this.atlasAsset = atlasAsset;
                this._onDragonBonesAssetLoaded(err, asset, atlasAsset);
            });
        });
    }
};
...
class ByteBuffer {
    ...
    set position(value) {
        if (value > this._length)
            throw "Out of bounds: set position";
        this._pos = value;
    }
    skip(count) {
        this._pos += count;
...
class UIPackage {
    ...
    get bundle() {
        var _a;
        return (_a = this._bundle) !== null && _a !== void 0 ? _a : resources;
    }
    get sprites() {
    ...
    constructor() {
        ...
        this._sprites = {};
        this._dependencies = [];
        this._addDependencyNames = [];
        this._branches = [];
        this._branchIndex = -1;
        this._loadingMode = UIPackageLoadingMode.Normal;
    }
    static get branch() {
        return _branch;
    }
    ...
    static loadPackage(...args) {
        ...
        bundle.load(path, Asset, onProgress, (err, asset) => {
            ...
            let pkg = this._touchPackage(path, asset, bundle);
            let cnt = pkg._items.length;
            let urls = [];
            let items = [];
            const loadingMode = pkg.loadingMode;
            for (var i = 0; i < cnt; i++) {
                var pi = pkg._items[i];
                if (loadingMode == UIPackageLoadingMode.Normal ||
                    loadingMode == UIPackageLoadingMode.Async) {
                    if (pi.type == PackageItemType.Atlas ||
                        pi.type == PackageItemType.Sound) {
            ...
            }
            {
                ...
                if (total > 0) {
                    if (pkg.loadingMode == UIPackageLoadingMode.Normal) {
                        const taskComplete = (err, item) => {
                            total--;
                            if (err)
                                lastErr = err;
                            if (total <= 0)
                        ...
                        items.forEach(i => {
                            if (i.aloneImage) {
                                ...
                                i.loadAsset(taskComplete);
                            }
                        });
                    }
                    else {
                        //UIPackageLoadingMode.Async 异步加载资源
                        items.forEach(i => i.loadAsset());
                        packageComplete();
                    }
                }
                else
    ...
    static tryLoadAloneImagePackage(path) {
        let p, _resolve, _reject;
        p = new Promise((resolve, reject) => {
            _resolve = resolve;
            _reject = reject;
        });
        let pkg = _instUniq[path];
        if (!!pkg) {
            const loadingMode = pkg.loadingMode;
            if (loadingMode == UIPackageLoadingMode.Normal ||
                loadingMode == UIPackageLoadingMode.Async) {
                let items = [];
                pkg._items.forEach(i => {
                    if (i.aloneImage) {
                        items.push(i);
                    }
                });
     ...
     * 加载的过程是异步的。
     */
    static preloadPackage(path, bundle = resources) {
        let p, _resolve, _reject;
        p = new Promise((resolve, reject) => {
            _resolve = resolve;
            _reject = reject;
        ...
        let pkg = _instUniq[path];
        if (!(pkg === null || pkg === void 0 ? void 0 : pkg._isValid)) {
            bundle.load(path, Asset, (err, asset) => {
                if (err) {
                    warn(err);
                    _reject(err);
                    return;
                }
                let _pkg = this._touchPackage(path, asset, bundle);
                if (!_pkg) {
                    _reject("preload error!!  the path: " + path);
                    return;
                }
                this._internalPreloadPackage(_pkg);
                _instById[_pkg.id] = _pkg;
                _instByName[_pkg.name] = _pkg;
                _instById[_pkg._path] = _pkg;
                _instUniq[path] = _pkg;
                _pkg._addPreLoadRef();
                _resolve(_pkg);
            });
            ...
            this._internalPreloadPackage(pkg);
            _resolve(pkg);
        }
        return p;
    }
    static _internalPreloadPackage(pkg) {
        pkg._items.forEach(item => item.loadAsset());
    }
    static removePackage(packageIdOrName) {
        ...
        if (pkg._path) {
            ...
            delete _instUniq[pkg._path];
        }
    }
    static removeAllPackage() {
        for (let name in _instByName) {
            this.removePackage(name);
        }
    }
    static createObject(pkgName, resName, userClass, useJobIfComponent) {
        var pkg = UIPackage.getByName(pkgName);
        if (pkg)
            return pkg.createObject(resName, userClass, useJobIfComponent);
        else
            return null;
    }
    static createObjectFromURL(url, userClass, useJobIfComponent) {
        var pi = UIPackage.getItemByURL(url);
        if (pi)
            return pi.owner.internalCreateObject(pi, userClass, useJobIfComponent);
        else
            return null;
    }
    ...
    /** 通过包名获取package的加载模式 */
    static convertLoadingMode(pkgName) {
        let mode = UIPackageLoadingMode.Normal;
        const matcher = pkgName.match(this._reg);
        if (matcher != void 0) {
            switch (matcher[1]) {
                case "Auto":
                case "auto":
                    mode = UIPackageLoadingMode.Auto;
                    break;
                case "Async":
                case "async":
                    mode = UIPackageLoadingMode.Async;
                    break;
            }
        }
        return mode;
    }
    static _touchPackage(path, asset, bundle) {
        let pkg = _instUniq[path];
        if (!pkg) {
            pkg = new UIPackage();
            pkg._bundle = bundle;
            ...
            if (!buffer) {
                console.warn("Look this  =>  path: " + path + " ,  bundle name: " + bundle.name);
                return null;
            }
    ...
    loadPackage(buffer, path) {
        ...
        this._loadingMode = UIPackage.convertLoadingMode(this._name);
        buffer.skip(20);
        ...
        for (i = 0; i < cnt; i++) {
            ...
            pi.id = buffer.readS();
            pi.aloneImage = (pi.type === PackageItemType.Atlas) && ToolSet.startsWith(pi.id, "atlas_");
    ...
    addRef() {
        if (checkIsResidentPkg(this._name) && this._ref) {
            return this;
        }
        if (this._preLoadRef > 0) {
            this._transAddRef();
        }
        else {
            this._ref++;
    ...
    _transAddRef() {
        if (checkIsResidentPkg(this._name))
            return this.addRef();
        if (this._preLoadRef > 0) {
            this._ref++;
            this._preLoadRef = 0;
    ...
    decRef() {
        ...
        if (UIPackage.getByName(this._name)) {
            UIPackage.removePackage(this._name);
            this._removeDependencyPackes();
        }
        let dependencies = this.dependencies;
        for (let i = 0, il = dependencies.length, pkg; i < il; i++) {
            pkg = _instById[dependencies[i].id];
            if (pkg && !checkIsResidentPkg(pkg.name)) {
                pkg.decRef();
            }
        }
        return this;
    }
    addDependencyName(pkgName) {
        if (pkgName && pkgName != this._name && !this._addDependencyNames.includes(pkgName)) {
            this._addDependencyNames.push(pkgName);
    ...
    _removeDependencyPackes() {
        if (this._addDependencyNames.length > 0) {
            for (let i = 0, il = this._addDependencyNames.length, pkg; i < il; i++) {
                pkg = UIPackage.getByName(this._addDependencyNames[i]);
                if (pkg && !checkIsResidentPkg(pkg.name)) {
                    pkg.decRef();
                }
            }
    ...
    createObject(resName, userClass, useJobIfComponent) {
        var pi = this._itemsByName[resName];
        if (pi)
            return this.internalCreateObject(pi, userClass, useJobIfComponent);
        else
            return null;
    }
    internalCreateObject(item, userClass, useJobIfComponent) {
        var g = Decls.UIObjectFactory.newObject(item, userClass);
        if (g == null)
            return null;
        constructingDepth.n++;
        if (useJobIfComponent && item.type == PackageItemType.Component)
            g["_isJobConstruct"] = true;
        g.beforeConstructEntry();
    ...
    getItemAssetByName(resName, onLoad) {
        var pi = this._itemsByName[resName];
        if (pi == null) {
            onLoad(new Error("Resource not found -" + resName), null);
        }
        this.getItemAsset(pi, onLoad);
    }
    getItemAsset(item, onLoad) {
        item.getAsset(onLoad);
    }
    /**
     * @deprecated
     */
    loadAllAssets(onLoad) {
     ...
     * @ 特殊需要先加载图片资源的情况使用
     */
    initLoadImagesAssets() {
        let cnt = this._items.length;
        const loadingMode = this.loadingMode;
        for (var i = 0; i < cnt; i++) {
            var pi = this._items[i];
            if (loadingMode == UIPackageLoadingMode.Normal ||
                loadingMode == UIPackageLoadingMode.Async) {
    ...
    addLoadedHandler(handler) {
        if (this._loadedHandlers == void 0)
            this._loadedHandlers = [handler];
        else
            this._loadedHandlers.push(handler);
    }
}
UIPackage._reg = /.*_([Aa]uto|[Aa]sync)$/;
...
class GTextField extends GObject {
    ...
    set font(value) {
        if (this._font != value || !value) {
            ...
            if (this.isFirstTimeVisible)
                this._onFontUpdate(value);
            else
                this._dirty |= 1 /* DirtyType.Font */;
        }
    }
    get fontSize() {
    ...
    set fontSize(value) {
        ...
        if (this._fontSize != value) {
            ...
            if (this.isFirstTimeVisible) {
                ...
                this._dirty |= 2 /* DirtyType.FontSize */;
        }
    }
    get color() {
        return this._color;
    ...
    set color(value) {
        ...
        if (this.isFirstTimeVisible) {
            ...
            this.updateGear(4);
            this._dirty |= 16 /* DirtyType.Color */;
        }
    }
    get align() {
    ...
    set leading(value) {
        if (this._leading != value) {
            this._leading = value;
            if (this._isFirstTimeEnabled) {
                this.markSizeChanged();
                this.updateFontSize();
            }
            else
                this._dirty |= 32 /* DirtyType.Leading */;
        }
    }
    get letterSpacing() {
    ...
    get stroke() {
        if (this._label.enableOutline)
            return this._label.outlineWidth;
        return 0;
    }
    set stroke(value) {
        if (this._outlineValue != value) {
            this._outlineValue = value;
            if (this.isFirstTimeVisible)
                this._updateStroke();
            else {
                this._dirty |= 64 /* DirtyType.Stroke */;
            }
        }
    }
    _updateStroke() {
        const value = this._outlineValue;
        if (value == 0)
            this._label.enableOutline = false;
        else {
    ...
    set strokeColor(value) {
        ...
        if (this.isFirstTimeVisible) {
            ...
            this.updateGear(4);
            this._dirty |= 4 /* DirtyType.StrokeColor */;
        }
    }
    get shadowOffset() {
    ...
    set shadowOffset(value) {
        ...
        if (this.isFirstTimeVisible)
            this._updateShadowOffset();
        else
            this._dirty |= 8 /* DirtyType.ShadowColor */;
    }
    _updateShadowOffset() {
        const offset = this._shadowOffset;
        if (offset.x != 0 || offset.y != 0) {
            this._label.enableShadow = true;
            this._label.shadowOffset.x = offset.x;
            this._label.shadowOffset.y = offset.y;
        }
        else {
            this._label.enableShadow = false;
        }
    }
    get shadowColor() {
    ...
    get shadowBlur() {
        return this._label.shadowBlur || 0;
    }
    set shadowBlur(val) {
        if (!this._label.enableShadow)
            return;
        this._label.shadowBlur = val;
    }
    ...
    get textWidth() {
        this.ensureSizeCorrect();
        return this._node.getComponent(UITransform).width;
    }
    ensureSizeCorrect() {
        if (this._sizeDirty) {
    ...
    textCountDown(option) {
        const { scheduler, getLeftSec, translateFunc, templateKey, endMsg, normalColor, endColor, interval } = option;
        const uuid = this.node.uuid;
        const handlerKey = `_gTextField.textCountDownHandler_${uuid}`;
        const normalColorVal = normalColor === null || normalColor === void 0 ? void 0 : normalColor._val;
        const endColorVal = endColor === null || endColor === void 0 ? void 0 : endColor._val;
        let handler;
        handler = scheduler[handlerKey];
        if (handler) {
            scheduler.unschedule(handler);
            delete scheduler[handlerKey];
        }
        let setupText = (content) => {
            if (templateKey)
                this.setVar(templateKey, content).flushVars();
            else
                this.text = content;
        };
        if (getLeftSec() <= 0) {
            setupText(endMsg !== null && endMsg !== void 0 ? endMsg : '已结束');
            if (endColorVal != void 0)
                this.color._val = endColorVal;
        }
        else {
            let curLeftSec;
            handler = (function () {
                const leftSec = getLeftSec();
                if (leftSec <= 0) {
                    setupText(endMsg !== null && endMsg !== void 0 ? endMsg : '已结束');
                    if (endColorVal != void 0)
                        this.color._val = endColorVal;
                ...
                else if (curLeftSec !== leftSec) {
                    curLeftSec = leftSec;
                    const timeStr = translateFunc ? translateFunc(leftSec) : '' + leftSec;
                    setupText(timeStr);
                }
            }).bind(this);
            if (normalColorVal != void 0)
                this.color._val = normalColorVal;
            scheduler.schedule(handler, interval !== null && interval !== void 0 ? interval : 0, macro.REPEAT_FOREVER);
            scheduler[handlerKey] = handler;
            handler();
        }
    }
    onFirstTimeVisible() {
        ...
        if (dirty & 34 /* DirtyType.MarkSizeChanged */)
            this.markSizeChanged();
        if (dirty & 1 /* DirtyType.UpdateFont */)
            this._onFontUpdate(this._font);
        if (dirty & 34 /* DirtyType.UpdateFontSize */)
            this.updateFontSize();
        if (dirty & 16 /* DirtyType.UpdateFontColor */)
            this.updateFontColor();
        if (dirty & 64 /* DirtyType.Stroke */)
            this._updateStroke();
        if (dirty & 68 /* DirtyType.UpdateStrokeColor */)
            this.updateStrokeColor();
        if (dirty & 128 /* DirtyType.ShadowOffset */)
            this._updateShadowOffset();
        if (dirty & 136 /* DirtyType.UpdateShadowColor */)
            this.updateShadowColor();
        this._dirty = 0 /* DirtyType.None */;
    }
    updateText() {
        ...
        if (this._isInitTextFlag && this._templateVars && (this._text.indexOf("{") != -1)) {
            this._label.string = "";
        }
        else {
    ...
    updateFontSize() {
        ...
        if (font instanceof BitmapFont) {
            ...
            if (fntConfig.resizable) {
                const temp = this._fontSize;
                this._label.fontSize = temp + 1;
                this._label.fontSize = this._fontSize = temp;
            }
            else {
    ...
    updateOverflow() {
        if (this._autoSize == AutoSizeType.Both)
            ...
            this._node.getComponent(UITransform).width = this._width;
        }
        else if (this._autoSize == AutoSizeType.Shrink) {
            this._label.overflow = Label.Overflow.SHRINK;
            this._node.getComponent(UITransform).setContentSize(this._width, this._height);
        }
        else {
    ...
    markSizeChanged() {
        ...
        if (this._autoSize == AutoSizeType.Both || this._autoSize == AutoSizeType.Height) {
            if (!this._sizeDirty) {
                this._node.emit(MyEvent.SIZE_DELAY_CHANGE);
    ...
    onLabelSizeChanged() {
        ...
        if (this._autoSize == AutoSizeType.Both || this._autoSize == AutoSizeType.Height) {
            ...
            this.setSize(this._node.getComponent(UITransform).width, this._node.getComponent(UITransform).height);
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        var str = buffer.readS();
        if (str != null) {
            this._isInitTextFlag = true;
            this.text = str;
            this._isInitTextFlag = false;
        }
    }
    _onFontUpdate(font) {
        this.markSizeChanged();
        let newFont = font ? font : UIConfig.defaultFont;
        if (newFont.startsWith("ui://")) {
            ...
            if (pi) {
                pi.getAsset((err, item) => {
                    this._realFont = item.asset;
                    this.updateFont();
                });
                newFont = null;
            }
...
class RichTextImageAtlas extends SpriteAtlas {
    getSpriteFrame(key) {
        ...
        if (pi) {
            if (pi.type == PackageItemType.Image)
                return pi.getAssetImmediately();
            else if (pi.type == PackageItemType.MovieClip)
                return pi.frames ? pi.frames[0].texture : null;
        }
...
class GRichTextField extends GTextField {
    ...
    updateText() {
        ...
        if (this._strokeV) {
            text2 = "<outline color=" + this._strokeColor.toHEX("#rrggbb") + " width=" + this._strokeV + ">" + text2 + "</outline>";
        }
        if (this._autoSize == AutoSizeType.Both) {
            if (this._richText.maxWidth != 0)
                this._richText["_maxWidth"] = 0;
            this._richText.string = text2;
            if (this.maxWidth != 0 && this._node.getComponent(UITransform).contentSize.width > this.maxWidth)
...
class InputProcessor extends Component {
    ...
    simulateClick(target) {
        let evt;
        evt = borrowEvent(MyEvent.TOUCH_BEGIN, true);
        ...
        evt.unuse();
        evt.type = MyEvent.TOUCH_END;
        evt.bubbles = true;
        target.node.dispatchEvent(evt);
        evt.unuse();
        evt.type = MyEvent.CLICK;
        evt.bubbles = true;
        target.node.dispatchEvent(evt);
        returnEvent(evt);
    }
    touchBeginHandler(evt) {
        ...
        evt.preventSwallow = ti.target != this._owner;
        this.setBegin(ti);
        let evt2 = this.getEvent(ti, ti.target, MyEvent.TOUCH_BEGIN, true);
        if (this._captureCallback)
    ...
    touchMoveHandler(evt) {
        ...
        this.handleRollOver(ti, ti.target);
        if (ti.began) {
            let evt2 = this.getEvent(ti, ti.target, MyEvent.TOUCH_MOVE, false);
            let done = false;
    ...
    touchEndHandler(evt) {
        let ti = this.updateInfo(evt.getID(), evt.getLocation());
        this.setEnd(ti);
        let evt2 = this.getEvent(ti, ti.target, MyEvent.TOUCH_END, false);
        let cnt = ti.touchMonitors.length;
        for (let i = 0; i < cnt; i++) {
            let mm = ti.touchMonitors[i];
        ...
        if (ti.target) {
            evt2 = this.getEvent(ti, ti.target, MyEvent.CLICK, true);
            ti.target.node.dispatchEvent(evt2);
            returnEvent(evt2);
        }
    ...
    mouseWheelHandler(evt) {
        ...
        let evt2 = this.getEvent(ti, ti.target, MyEvent.MOUSE_WHEEL, true);
        ti.target.node.dispatchEvent(evt2);
        returnEvent(evt2);
    }
    ...
    handleRollOver(ti, target) {
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            if (element.node && element.node.activeInHierarchy) {
                let evt = this.getEvent(ti, element, MyEvent.ROLL_OUT, false);
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            if (element.node && element.node.activeInHierarchy) {
                let evt = this.getEvent(ti, element, MyEvent.ROLL_OVER, false);
                element.node.dispatchEvent(evt);
                returnEvent(evt);
            }
        }
...
class Controller extends EventTarget {
    ...
    set selectedIndex(value) {
        if (this._selectedIndex != value) {
            ...
            this.emit(MyEvent.STATUS_CHANGED, this);
            this.changing = false;
        }
    }
    onChanged(callback, thisArg) {
        this.on(MyEvent.STATUS_CHANGED, callback, thisArg);
    }
    offChanged(callback, thisArg) {
        this.off(MyEvent.STATUS_CHANGED, callback, thisArg);
    }
...
class ScrollPane extends Component {
    constructor() {
        super(...arguments);
        this._aniFlag = 0;
        this._moveCaptureTouch = false;
    }
    onLoad() {
        this.node.layer = UIConfig.defaultUILayer;
    }
    setup(buffer) {
        ...
        o.on(MyEvent.TOUCH_BEGIN, this.onTouchBegin, this);
        o.on(MyEvent.TOUCH_MOVE, this.onTouchMove, this);
        o.on(MyEvent.TOUCH_END, this.onTouchEnd, this);
        o.on(MyEvent.MOUSE_WHEEL, this.onMouseWheel, this);
        ...
        if (scrollBarDisplay != ScrollBarDisplayType.Hidden) {
            ...
            if (this._scrollBarDisplayAuto) {
                ...
                o.on(MyEvent.ROLL_OVER, this.onRollOver, this);
                o.on(MyEvent.ROLL_OUT, this.onRollOut, this);
            }
        }
        if (headerRes) {
    ...
    set viewWidth(value) {
        ...
        this._owner.width = this._viewSize.x = value;
    }
    get viewHeight() {
    ...
    set viewHeight(value) {
        ...
        this._owner.height = this._viewSize.y = value;
    }
    get currentPageX() {
        if (!this._pageMode)
            return 0;
    ...
    get footerLockedSize() {
        return this._footerLockedSize;
    }
    onOwnerSizeChanged() {
    ...
    handleSizeChanged() {
        ...
        (_a = this._maskContainer.getComponent(UITransform)) === null || _a === void 0 ? void 0 : _a.setContentSize(maskWidth, maskHeight);
        if (this._vtScrollBar)
    ...
    refresh(dt) {
        ...
        this.refresh2();
        this._owner.node.emit(MyEvent.SCROLL, this._owner);
    ...
    setMoveCaptureTouchFlag(flag) {
        this._moveCaptureTouch = flag;
    }
    onTouchMove(evt) {
        ...
        if (ScrollPane.draggingPane && ScrollPane.draggingPane != this && ScrollPane.draggingPane._owner.onStage)
            return;
        if (this._moveCaptureTouch && this.footerLockedSize > 0) {
            this.lockFooter(0);
            evt.upCaptureTouch();
            return;
        }
        ...
        */
        ...
        if (this._pageMode)
            this.updatePageController();
        this._owner.node.emit(MyEvent.SCROLL);
    }
    onTouchEnd(evt) {
        if (ScrollPane.draggingPane == this)
            ScrollPane.draggingPane = null;
        ...
        if (flag) {
            ...
            if (this._tweenChange.x < -UIConfig.touchDragSensitivity || this._tweenChange.y < -UIConfig.touchDragSensitivity) {
                ...
                this._owner.node.emit(MyEvent.PULL_DOWN_RELEASE), this._owner;
                this._refreshEventDispatching = false;
            }
            else if (this._tweenChange.x > UIConfig.touchDragSensitivity || this._tweenChange.y > UIConfig.touchDragSensitivity) {
                this._refreshEventDispatching = true;
                this._owner.node.emit(MyEvent.PULL_UP_RELEASE, this._owner);
                this._refreshEventDispatching = false;
            }
    ...
    killTween() {
        ...
        this._tweening = 0;
        this.updateScrollBarVisible();
        this._owner.node.emit(MyEvent.SCROLL_END, this._owner);
    }
    checkRefreshBar() {
...
var _gestureFlag = 0;
const TWEEN_TIME_GO = 0.4; //调用SetPos(ani)时使用的缓动时间
const TWEEN_TIME_DEFAULT = 0.2; //惯性滚动的最小缓动时间
...
class Transition {
    ...
    onDelayedPlay() {
        if (!!this._startHook && this._owner) {
            this._startHook.apply(this, [this._owner]);
        }
        this.internalPlay();
            ...
            case ActionType.Sound:
                if (this._playing && item.time >= this._startTime) {
                    if (value.audioClip == null) {
                        ...
                        if (pi) {
                            const sound = pi.getAssetImmediately();
                            value.audioClip = sound;
                        }
                    }
                    if (value.audioClip)
...
class UserData {
    static set(pkgId, pkgItemId, objId, data) {
        let info = this.info;
        let pkg = info[pkgId];
        if (!pkg)
            pkg = info[pkgId] = Object.create(null);
        let pkgItem = pkg[pkgItemId];
        if (!pkgItem)
            pkgItem = pkg[pkgItemId] = Object.create(null);
        pkgItem[objId] = data;
    }
    static get(pkgId, pkgItemId, objId) {
        let info = this.info;
        let pkg = info[pkgId];
        if (!pkg)
            return null;
        ...
        if (!pkgItem)
            return null;
        return pkgItem[objId];
    }
    static parseCustomData(baseUserData, customData) {
        let res = {};
        if (!!baseUserData)
            res = parseQueryString(res, baseUserData, "\n");
        if (!!customData)
            res = parseQueryString(res, customData, "\n");
        return res;
    }
    static setHandlers(handlers) {
        this.handlers = handlers;
    }
    static applyData(obj, data) {
        if (!data)
            return;
        const handlers = this.handlers;
        for (let k in data) {
            if (handlers[k] != null)
                handlers[k](data[k], obj, data);
        }
    }
    static buildUserData(component) {
        const baseUserData = component.packageItem && component.baseUserData;
        if (!!baseUserData) {
            const contentItem = component.packageItem.getBranch();
            let userData = this.get(contentItem.owner.id, contentItem.id, "$");
            if (!userData) {
                userData = this.parseCustomData(baseUserData, "");
                this.set(contentItem.owner.id, contentItem.id, "$", userData);
            }
            component.userData = userData;
            this.applyData(component, userData);
        }
    }
}
UserData.info = Object.create(null);
UserData.handlers = Object.create(null);
function parseQueryString(out, str, separator = "&") {
...
class GComponent extends GObject {
    /** 是否为作业创建 */
    get isJobConstruct() { return this._isJobConstruct; }
    /** 是否已经完成创建 */
    get loaded() { return this._loaded; }
    get ccRenderer() { return this._ccRenderer; }
    set ccRenderer(val) {
        if (this._isJobConstruct && !this._loaded) {
            this._waitingRenderer = val;
            return;
        }
        super.setCCRenderer(val);
    }
    get data() { return this._data; }
    set data(d) {
        if (this._isJobConstruct && !this._loaded) {
            this._itemDataDirty = d;
            return;
        }
        this.setObjectData(d);
    }
    constructor() {
        super();
        this._sortingChildCount = 0;
        this._isJobConstruct = false;
        this._originXY = { x: 0, y: 0 };
        this._originSize = { width: 0, height: 0 };
    ...
    dispose() {
        ...
        if (this._scrollPane) {
            this._scrollPane.cancelDragging();
    ...
    addAsyncComponentRenderer(val, params) {
        if (this._isJobConstruct && !this._underConstruct) {
            if (!this._waitingExtraRenderers)
                this._waitingExtraRenderers = [];
            this._waitingExtraRenderers.push(val, params);
        }
        else
            this.addRenderer(val, params);
    }
    ...
    onChildAdd(child, index) {
        ...
        if (this._childrenRenderOrder == ChildrenRenderOrder.Ascent)
            ...
            child.node.setSiblingIndex(cnt - index - 1);
        else
            this._partner.callLater(this.buildNativeDisplayList);
    }
    ...
    setMask(value, inverted) {
        ...
        if (this._maskContent) {
            ...
            if (this._maskContent instanceof GImage && !this._maskContent._content.spriteFrame) {
            ...
            //     this.onMaskReady();
            // else
            //     this.on(MyEvent.DISPLAY, this.onMaskReady, this);
    ...
    onMaskReady() {
        this.off(MyEvent.DISPLAY, this.onMaskReady, this);
    ...
    handleSizeChanged() {
        ...
        if (this._scrollPane)
            ...
            this._container.getComponent(UITransform).setContentSize(this.viewWidth, this.viewHeight);
    }
    handleGrayedChanged() {
    ...
    _hitTest(pt, globalPt) {
        ...
        if (this.hitArea) {
            ...
            let clippingSize = this._container.getComponent(UITransform).contentSize;
    ...
    beforeConstructEntry() {
        this._checkUseJobBeforeConstruct();
        if (this._isJobConstruct) {
            this._createSubContainer();
            const buffer = this._getDecodedPackageItem().rawData;
            const oldPos = buffer.position;
            buffer.seek(0, 0);
            this._initBase(buffer);
            buffer.position = oldPos;
        }
    }
    constructFromResource() {
        if (this._isJobConstruct) {
            if (!this._asyncStandBy)
                jobSystem.addJob(() => this.constructFromResource2(null, 0));
        }
        else
    ...
    constructFromResource2(objectPool, poolIndex) {
        ...
        if (!isValid(this._node))
            return;
        var contentItem = this._getDecodedPackageItem();
        var i;
        var dataLen;
        var curPos;
        var nextPos;
        ...
        buffer.seek(0, 0);
        if (this._isJobConstruct)
            //为了保证异步组件对自身属性的设置在setup_beforeAdd之前
            //在被初始化但没有constructFromResource之前就先行initBase
            //那么在异步执行constructFromResource2的时候，就不需要再次设置了
            this._skipBase(buffer);
        else
            this._initBase(buffer);
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (objectPool) {
                ...
                if (pi) {
                    ...
                    child._taskId = this._taskId;
                    if (type == ObjectType.Component && child instanceof GComponent) {
                        child._setup_beforeJobCompLoad(buffer, curPos);
                        if (child._isJobConstruct) {
                            child._asyncStandBy = true;
                            child._createSubContainer();
                        }
                    }
            ...
            if (!!child.customData) {
                let userData = UserData.get(contentItem.owner.id, contentItem.id, child.id);
                if (!userData) {
                    userData = UserData.parseCustomData(pi ? child.asCom.baseUserData : "", child.customData);
                    UserData.set(contentItem.owner.id, contentItem.id, child.id, userData);
                }
                child.userData = userData;
                UserData.applyData(child, userData);
            }
            child._parent = this;
            if (this._isJobConstruct && isValid(this._subContainer))
                child.node.parent = this._subContainer;
            else
                child.node.parent = this._container;
            this._children.push(child);
            buffer.position = curPos + dataLen;
        ...
        if (!!this.baseUserData) {
            let thizUserData = UserData.get(contentItem.owner.id, contentItem.id, this.id);
            if (!thizUserData) {
                thizUserData = UserData.parseCustomData(this.baseUserData, "");
                UserData.set(contentItem.owner.id, contentItem.id, this.id, thizUserData);
            }
            this.userData = thizUserData;
            UserData.applyData(this, thizUserData);
        }
        buffer.seek(0, 3);
        ...
        if (this._waitingRenderer)
            this.setCCRenderer(this._waitingRenderer);
        if (this._waitingExtraRenderers) {
            const waitings = this._waitingExtraRenderers;
            for (let i = 0, len = waitings.length; i < len; i += 2) {
                this.addRenderer(waitings[i], waitings[i + 1]);
            }
            this._waitingExtraRenderers = null;
        }
        this._loaded = true;
        if (this._isJobConstruct) {
            this._subContainer.parent = this._container;
            (_a = this.node) === null || _a === void 0 ? void 0 : _a.emit(MyEvent.LOADED);
            this._setup_afterJobCompLoad();
            if (this._itemDataDirty) {
                const d = this._itemDataDirty;
                this._itemDataDirty = null;
                this.setObjectData(d);
            }
            this.onConstruct();
        }
    ...
    onEnable() {
        ...
        let cnt1 = this._controllers.length;
        for (let i = 0; i < cnt1; ++i) {
            this._controllers[i].onEnable();
        }
        if (this._modleEnable)
            this._modleEnable.apply(this);
    }
    onDisable() {
        super.onDisable();
        let cnt = this._transitions.length;
        ...
        let cnt1 = this._controllers.length;
        for (let i = 0; i < cnt1; ++i) {
            this._controllers[i].onDisable();
        }
        if (this._modleDisable)
            this._modleDisable.apply(this);
    }
    onFirstTimeVisible() {
        if (this._isJobConstruct && this._asyncStandBy) {
            this._asyncStandBy = false;
            this.constructFromResource();
        }
    }
    set observerModleEnable(fun) {
        this._modleEnable = fun;
    }
    set observerModleDisable(fun) {
        this._modleDisable = fun;
    }
    _getDecodedPackageItem() {
    ...
    /** 跳过基础信息数据 */
    _skipBase(buffer) {
        //对照GComponent._initBase，读了多少字节就跳过多少字节
        buffer.skip(2 * 4);
        if (buffer.readBool())
            buffer.skip(4 * 4);
        if (buffer.readBool())
    ...
    _checkUseJobBeforeConstruct() {
        const buffer = this._getDecodedPackageItem().rawData;
        const oldPos = buffer.position;
        buffer.seek(0, 4);
        const baseUserData = buffer.readS();
        if (baseUserData != void 0) {
            const matcher = baseUserData.match(GComponent._asyncTagReg);
            this._isJobConstruct = !!matcher;
        }
        buffer.position = oldPos;
    }
    _createSubContainer() {
        const subContainer = this._subContainer = new Node("SubContainer");
        const transform = subContainer.addComponent(UITransform);
        transform.setAnchorPoint(0, 1);
        return subContainer;
    }
    _setup_beforeJobCompLoad(parentBuffer, beginPos) {
        /**
         * see GObject.setup_beforeAdd, skip all data before customData
         * readS -> 2B, readInt -> 4B, readBool -> 1B, readFloat -> 4B, readByte -> 1B
         */
        const oldPos = parentBuffer.position;
        const buffer = this._getDecodedPackageItem().rawData;
        const oldPos2 = buffer.position;
        try {
            let userData;
            let baseUserData;
            buffer.seek(0, 4);
            baseUserData = buffer.readS();
            parentBuffer.seek(beginPos, 0);
            parentBuffer.skip(5);
            parentBuffer.skip(2 * 2);
            parentBuffer.skip(2 * 4);
            if (parentBuffer.readBool())
                parentBuffer.skip(2 * 4);
            if (parentBuffer.readBool())
                parentBuffer.skip(4 * 4);
            if (parentBuffer.readBool())
                parentBuffer.skip(2 * 4);
            if (parentBuffer.readBool())
                parentBuffer.skip(2 * 4);
            if (parentBuffer.readBool())
                parentBuffer.skip(2 * 4 + 1);
            parentBuffer.skip(2 * 4);
            // this._asyncStandBy = parentBuffer.readBool();
            parentBuffer.skip(5);
            userData = parentBuffer.readS();
            do {
                if (baseUserData != void 0) {
                    const matcher = baseUserData.match(GComponent._asyncTagReg);
                    this._isJobConstruct = !!matcher;
                ...
                if (this._isJobConstruct)
                    break;
                if (userData != void 0) {
                    const matcher = userData.match(GComponent._asyncTagReg);
                    this._isJobConstruct = !!matcher;
                }
            } while (false);
        }
        catch (e) {
            console.log("_precheck custom data error");
        }
        finally {
            parentBuffer.position = oldPos;
            buffer.position = oldPos2;
        }
        {
            //在此处预先使用initBase，取代constructFromResource2中的相同调用
            const contentItem = this._getDecodedPackageItem();
            const myBuffer = contentItem.rawData;
            const oldPos = myBuffer.position;
            myBuffer.seek(0, 0);
            this._initBase(myBuffer);
            this._recordOriginXY();
            this._recordOriginSize();
            myBuffer.position = oldPos;
        }
    }
    _setup_afterJobCompLoad() {
        // const {
        //     x: ox, y: oy
        // } = this._originXY;
        // if (this._x != ox || this._y != oy)
        //     adaptEvent.asDelayAdaptPositionEvt()
        //         .setPositionChange(this._x, this._y, ox, oy)
        //         .dispatch(this._node);
        const { width: ow, height: oh } = this._originSize;
        if (this._width != ow || this._height != oh)
            adaptEvent.asDelayAdaptSizeEvt()
                .setSizeChange(this._width, this._height, ow, oh)
                .dispatch(this._node);
    }
    _recordOriginXY() {
        const xy = this._originXY;
        xy.x = this._x;
        xy.y = this._y;
    }
    _recordOriginSize() {
        const size = this._originSize;
        size.width = this.initWidth;
        size.height = this.initHeight;
    }
}
GComponent._asyncTagReg = /\b[Aa]sync=(True|true)\b/;
...
class Window extends GComponent {
    constructor() {
        ...
        this._node.on(MyEvent.TOUCH_BEGIN, this.onTouchBegin_1, this, true);
    }
    addUISource(source) {
    ...
    set dragArea(value) {
        if (this._dragArea != value) {
            if (this._dragArea) {
                ...
                this._dragArea.off(MyEvent.DRAG_START, this.onDragStart_1, this);
            }
            this._dragArea = value;
            if (this._dragArea) {
                this._dragArea.draggable = true;
                this._dragArea.on(MyEvent.DRAG_START, this.onDragStart_1, this);
            }
        }
    }
    get contentArea() {
...
class DragAgent extends GComponent {
    constructor() {
        super();
        this._offsetGlobalX = 0;
        this._offsetGlobalY = 0;
        this._loader = new GLoader();
        this._loader.setSize(100, 100);
        this._loader.setPivot(0.5, 0.5, true);
        this._loader.align = AlignType.Center;
        this._loader.verticalAlign = VertAlignType.Middle;
    }
    set thatRenderer(val) {
        this._tempRender = val;
        if (!!this._obj)
            return;
        if (!!val) {
            this._loader.ccRenderer = this._tempRender;
        }
        else {
            this._loader.ccRenderer = null;
        }
    }
    set sourceData(val) {
        ...
        if (!!this._obj) {
            if (!!this._tempRender) {
                this._obj.node.getComponent(this._tempRender).data = val;
            }
            return;
        }
        this._loader.data = val;
    }
    get sourceData() {
    ...
    set offsetGlobalX(v) {
        this._offsetGlobalX = v;
    }
    get offsetGlobalX() {
        return this._offsetGlobalX;
    }
    set offsetGlobalY(v) {
        this._offsetGlobalY = v;
    }
    get offsetGlobalY() {
        return this._offsetGlobalY;
    }
    clean() {
...
class DragDropManager {
    ...
    constructor() {
        this._offYSize = 0;
        this._agent = new DragAgent();
        this._agent.draggable = true;
        this._agent.touchable = false;
        this._agent.setSize(100, 100);
        this._agent.setPivot(0.5, 0.5, true);
        this._agent.sortingOrder = 1000000;
        this._agent.on(MyEvent.DRAG_END, this.onDragEnd, this);
    }
    get dragAgent() {
        return this._agent;
    }
    get dragAgentObj() {
        var _a;
        return (_a = this._agent.obj) !== null && _a !== void 0 ? _a : this._agent.loader;
    }
    get dragging() {
        return this._agent.parent != null;
    }
    startDrag(icon, renderCtor, sourceData, touchId, offsetGlobalX = 0, offsetGlobalY = 0, delayMis) {
        if (this._agent.parent)
            return;
        this._sourceData = sourceData;
        if (!!renderCtor)
            this._agent.ccRenderer = renderCtor;
        else
            this._agent.ccRenderer = null;
        GRoot.inst.addChild(this._agent);
        this._agent.url = icon;
        this._agent.offsetGlobalX = offsetGlobalX;
        this._agent.offsetGlobalY = offsetGlobalY;
        this._agent.data = sourceData;
    ...
    startDragObj(obj, renderCtor, sourceData, touchId, offsetGlobalX = 0, offsetGlobalY = 0, delayMis) {
        if (this._agent.parent)
            return;
        this._agent.obj = obj;
        if (!!renderCtor)
            this._agent.thatRenderer = renderCtor;
        else
            this._agent.thatRenderer = null;
        GRoot.inst.addChild(this._agent);
        this._sourceData = this._agent.sourceData = sourceData;
        this._agent.offsetGlobalX = offsetGlobalX;
        this._agent.offsetGlobalY = offsetGlobalY;
        let pt = GRoot.inst.getTouchPosition(touchId);
        pt = GRoot.inst.globalToLocal(pt.x, pt.y);
    ...
    get offYSize() { return this._offYSize; }
    set offYSize(v) {
        GObject.draggingOffSet = v;
        this._offYSize = v;
    }
    cancel() {
        if (this._agent.parent) {
    ...
    onDragEnd() {
        ...
        while (obj) {
            if (obj.node.hasEventListener(MyEvent.DROP)) {
                obj.requestFocus();
                obj.node.emit(MyEvent.DROP, obj, sourceData);
                return;
            }
            obj = obj.parent;
        }
...
class GRoot extends GComponent {
    ...
    static create(rootParent) {
        GRoot._inst = new GRoot();
        rootParent.addChild(GRoot._inst.node);
    ...
    constructor() {
        ...
        if (!EDITOR) {
            ...
            window.addEventListener("safearea-change", this._safeAreaChange);
        }
        let musicNode = new Node("musicNode");
        this._audioEngine = musicNode.addComponent(AudioSourceComponent);
        let soundEffNode = new Node("soundEffNode");
        this._oneShotAudioEngine = soundEffNode.addComponent(AudioSourceComponent);
        this.node.addChild(musicNode);
        this.node.addChild(soundEffNode);
    }
    onDestroy() {
        ...
        if (!EDITOR) {
            ...
            window.removeEventListener("safearea-change", this._safeAreaChange);
        }
        if (this == GRoot._inst)
            GRoot._inst = null;
    }
    _safeAreaChange() {
        ...
        this._timeoutId = setTimeout(() => {
            if (GRoot._inst != null) {
                sys.getSafeAreaRect();
                this.safeAreaRect.set(sys.getSafeAreaRect());
                this._timeoutId = null;
                this.node.emit("SafeArea");
            }
        }, 10);
    }
    getTouchPosition(touchId) {
    ...
    showPopup(popup, target, dir) {
        ...
        popup.setPosition(pt.x, pt.y + DragDropManager.inst.offYSize);
    }
    ...
    playOneShotSound(clip, volumeScale) {
        if (!this._oneShotAudioEngine)
            return;
        if (volumeScale === undefined)
            volumeScale = 1;
        if (this._oneShotAudioEngine.isValid) {
            this._oneShotAudioEngine.clip = clip;
            this._oneShotAudioEngine.volume = this._volumeScale * volumeScale;
            this._oneShotAudioEngine.loop = false;
            this._oneShotAudioEngine.play();
        }
    }
    get oneShotAudioEngine() {
        return this._oneShotAudioEngine;
    }
    get audioEngine() {
...
class GTextInput extends GTextField {
    ...
    createRenderer() {
        ...
        this.on(MyEvent.TOUCH_END, this.onTouchEnd1, this);
    }
    _delayCreate() {
        var _a;
        let thiz = this;
        this._partner.scheduleOnce(() => {
            var _a;
            if ((_a = thiz._node) === null || _a === void 0 ? void 0 : _a.isValid) {
                thiz._editBox = thiz._node.addComponent(MyEditBox);
                thiz._editBox.maxLength = -1;
                thiz._editBox["_updateTextLabel"]();
                thiz._delaySetup();
            }
        }, (_a = UIConfig.textInputDelayInitSec) !== null && _a !== void 0 ? _a : 0.21);
    }
    set editable(val) {
        if (!this._editBox) {
            this._tempEditableArg = val;
            return;
        }
    ...
    set verticalAlign(value) {
        if (!this._editBox) {
            this._tempverticalAlignArg = value;
            return;
        }
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (str != null)
            this.promptText = str;
        else if (this._editBox && this._editBox.placeholderLabel)
    ...
    _delaySetup() {
        var _a;
        if (!!this._tempPromptTextArg) {
            this.promptText = this._tempPromptTextArg;
        }
        if (!this.promptText && this._editBox.placeholderLabel)
            this._editBox.placeholderLabel.string = "";
        this.restrict = this._setupArg2;
        this.singleLine = this._tempSingleLineArg;
        this.verticalAlign = this._tempverticalAlignArg;
        this.align = this._tempAlignArg;
        this.password = this._tempPasswordArg;
        this.editable = this._tempEditableArg;
        if (!!this._tempMaxLengthArg) {
            this.maxLength = this._tempMaxLengthArg;
        }
        //同步一下对齐方式
        if (this._editBox.placeholderLabel) {
        ...
        this.updateFontColor();
        this.updateFontSize();
        (_a = this._node.getComponent(MyEditBox)) === null || _a === void 0 ? void 0 : _a.updateView();
    }
}
class MyEditBox extends EditBox {
    updateView() {
        ...
        if (impl) {
            //@ts-ignore
            (_a = impl["_edTxt"]) === null || _a === void 0 ? void 0 : _a.style.overflowY = "hidden";
            //@ts-ignore
            if (impl["_updateMatrix"] && (typeof impl["_updateMatrix"] === "function"))
                impl["_updateMatrix"]();
        }
    }
    _registerEvent() {
    ...
    // if (this.textLabel)
    //     this.textLabel.node.getComponent(UITransform).setContentSize(size.width, size.height);
    // if (this.placeholderLabel)
    //     this.placeholderLabel.node.getComponent(UITransform).setContentSize(size.width, size.height);
    // }
    openKeyboard() {
        let impl = this["_impl"];
        if (impl) {
...
class GObjectPool {
    ...
    getObject(url, useJobIfComponent) {
        ...
        var child = UIPackage.createObjectFromURL(url, null, useJobIfComponent);
        return child;
    }
    returnObject(obj) {
...
class CCNodeLinker {
    ...
    setTarget(target) {
        if (isValid(this._target)) {
            this._target.parent = void 0;
            this._target.destroy();
            this._target = void 0;
        }
        if (isValid(target) && isValid(this._root)) {
            this._target = target;
            target.parent = this._root;
        }
    }
}
...
function getOrAddComponent(target, type) {
...
class InternalResKeeper extends Component {
    getRes() {
        return this._asset;
    }
    setRes(asset) {
        asset === null || asset === void 0 ? void 0 : asset.addRef();
        if (isValid(this._asset))
            this._asset.decRef();
        this._asset = asset;
    }
    onDestroy() {
...
class HashLinkedList {
    ...
    get head() {
        return this._size > 0 ? this._head.next.data : null;
    }
    get tail() {
        return this._size > 0 ? this._tail.prev.data : null;
    }
    constructor() {
        this._dic = {};
        const head = this._head = new HashLinkedListNode();
        const tail = this._tail = new HashLinkedListNode();
        head.next = tail;
        tail.prev = head;
        this._size = 0;
    }
    get(id) {
        var _a, _b;
        return (_b = (_a = this._dic[id]) === null || _a === void 0 ? void 0 : _a.data) !== null && _b !== void 0 ? _b : null;
    }
    add(data) {
        const node = HashLinkedListNode.pool.borrow();
        node.data = data;
    ...
    delete(keyOrData) {
        let key;
        if (typeof keyOrData == "number")
            key = keyOrData;
        else
            key = keyOrData.hashKey;
        const node = this._dic[key];
        if (node) {
            this._deleteNode(node);
            HashLinkedListNode.pool.returns(node);
            --this._size;
        }
    }
    popHead() {
    ...
    _appendNode(node) {
        ...
        const tailPrev = tail.prev;
        tailPrev.next = node;
        node.prev = tailPrev;
        tail.prev = node;
        node.next = tail;
        this._dic[node.data.hashKey] = node;
    }
    _deleteNode(node) {
        ...
        node.next = null;
        node.prev = null;
        delete this._dic[node.data.hashKey];
    }
}
class HashLinkedListNode {
}
HashLinkedListNode.pool = new Pool(HashLinkedListNode, null, node => {
    node.data = node.prev = node.next = null;
});

class JobSystem extends EventTarget {
    get id() { return this._id; }
    get status() { return this._status; }
    get curJob() { return this.getJobById(this._curJobId); }
    constructor(option) {
        var _a, _b;
        super();
        this._internalJobId = 0;
        this._nextTime = 0;
        this._jobKey2JobId = {};
        this._nextJobCall = () => {
            this._nextTimerId = undefined;
            this._startTime = performance.now();
            this._nextJob();
        };
        this._update = () => {
            const curTime = performance.now();
            if (curTime < this._nextTime)
                return;
            this._startTime = curTime;
            this._nextJob();
        };
        this._id = ++JobSystem._sysId;
        this._status = JobSystem.kStatusIdle;
        this._runners = new HashLinkedList();
        this._jobInterval = typeof (option === null || option === void 0 ? void 0 : option.intervalMs) == "function" ?
            option.intervalMs() :
            (_a = option === null || option === void 0 ? void 0 : option.intervalMs) !== null && _a !== void 0 ? _a : 0;
        this._slotDuration = typeof (option === null || option === void 0 ? void 0 : option.slotDurationMs) == "function" ?
            option.slotDurationMs() :
            (_b = option === null || option === void 0 ? void 0 : option.slotDurationMs) !== null && _b !== void 0 ? _b : (1 / (+game.frameRate) * 1000 * 0.15);
    }
    addJob(...args) {
        var _a;
        const jobId = ++this._internalJobId;
        let newJob;
        let jobKey;
        if (args[0] instanceof Function) {
            newJob = Job.pool.borrow();
            newJob.extraExecute = args[0];
            newJob.timeout = (_a = args[1]) !== null && _a !== void 0 ? _a : Job.kDefaultTimeout;
            newJob.onTimeout = args[2];
            newJob.jobKey = jobKey = args[3];
        }
        else {
            newJob = args[0];
        ...
        if (jobKey)
            this.terminateByKey(jobKey);
        const runner = JobRunner.pool.borrow();
        runner.status = 0 /* RunnerStatus.Waiting */;
        runner.job = newJob;
        runner.jobId = jobId;
        this._runners.add(runner);
        if (this._status === JobSystem.kStatusIdle) {
            this._changeStatus(JobSystem.kStatusBusy);
        }
        return jobId;
    }
    /**
    ...
    next() {
        if (this._nextTimerId)
            return;
        if (this._startTime)
            log(`job system ${this._id}:previous jobs cost ${this._endTime - this._startTime}`);
        if (this._jobInterval > 0)
            this._nextTimerId = setTimeout(this._nextJobCall, this._jobInterval);
        else
            this._nextTimerId = setTimeout(this._nextJobCall);
    }
    terminate(jobId) {
        const jobRunner = this._runners.get(jobId);
        if (jobRunner && jobRunner.status == 0 /* RunnerStatus.Waiting */) {
            jobRunner.status = 2 /* RunnerStatus.Ended */;
            JobRunner.pool.returns(jobRunner);
        }
    }
    terminateByKey(jobKey) {
        const jobId = this._jobKey2JobId[jobKey];
        if (jobId != void 0)
            return this.terminate(jobId);
    }
    getJobById(jobId) {
        var _a;
        return (_a = this._runners.get(jobId)) === null || _a === void 0 ? void 0 : _a.job;
    }
    getJobByKey(jobKey) {
        const jobId = this._jobKey2JobId[jobKey];
        if (jobId != void 0)
    ...
    _changeStatus(status) {
        ...
        this._status = status;
        this.emit(JobSystem.EventType.STATUS_CHANGED, status, oldStatus);
        if (oldStatus == JobSystem.kStatusBusy)
            director.off(Director.EVENT_AFTER_UPDATE, this._update);
        if (status == JobSystem.kStatusBusy) {
            director.on(Director.EVENT_AFTER_UPDATE, this._update);
        }
    }
    _nextJob() {
        if (this.status != JobSystem.kStatusBusy && this.status != JobSystem.kStatusClosed)
            return;
        if (this._timerId) {
        ...
        let runner = runners.popHead();
        while (runner && runner.status == 2 /* RunnerStatus.Ended */)
            runner = runners.popHead();
        if (!runner) {
            if (this._status == JobSystem.kStatusBusy)
                this._changeStatus(JobSystem.kStatusIdle);
            else
                this._changeStatus(JobSystem.kStatusClosed);
            this._startTime = 0;
            return;
        }
        const { job, jobId } = runner;
        if (job.onTimeout) {
            const timeout = job.timeout || Job.kDefaultTimeout;
            this._timerId = setTimeout(() => {
                if (job.onTimeout)
                    job.onTimeout.call(job, jobId);
                this._yieldJob();
            }, timeout);
        }
        this._curJobId = jobId;
        runner.status = 1 /* RunnerStatus.Running */;
        try {
            if (job.jobKey) {
                delete this._jobKey2JobId[job.jobKey];
            }
            const result = job.onExecute.call(job, jobId);
            const slotDuration = this._slotDuration;
            if (result instanceof Promise)
                result.then(() => {
                    runner.status = 2 /* RunnerStatus.Ended */;
                    JobRunner.pool.returns(runner);
                    this._endTime = performance.now();
                    if (this._endTime - this._startTime >= slotDuration)
                        this._yieldJob();
                    else
                        this._nextJob();
                });
    ...
    _yieldJob() {
        if (this._startTime && DEBUG)
            log(`job system ${this._id}:previous jobs cost ${this._endTime - this._startTime}`);
        this._nextTime = performance.now() + this._jobInterval;
    }
}
/** idle, welcome new jobs */
JobSystem.kStatusIdle = 0;
/** busy, working for some jobs */
JobSystem.kStatusBusy = 1;
/** closed, closed and would not receive job any more and is processing the remaining jobs now */
JobSystem.kStatusClosed = 2;
/** drained, ready to be destroy */
JobSystem.kStatusDrained = 3;
JobSystem.EventType = {
    STATUS_CHANGED: "status-changed"
};
JobSystem._sysId = 0;
const jobSystem = new JobSystem();
/** 微型作业队列，用来加载图片等相对没那么耗时的操作，使得图片能够更快的展示出来 */
const microJobs = new JobSystem();
class Job {
    constructor() {
        this.timeout = Job.kDefaultTimeout;
    }
    onExecute(jobId) {
        this.extraExecute.call(this, jobId);
    }
}
Job.kDefaultTimeout = 3000;
Job.pool = new Pool(Job, null, job => {
    job.timeout = Job.kDefaultTimeout;
    job.extraExecute = null;
});
class JobRunner {
    get hashKey() { return this.jobId; }
}
JobRunner.pool = new Pool(JobRunner, null, runner => {
    const job = runner.job;
    runner.job = null;
    runner.jobId = undefined;
    if (job instanceof Job)
        Job.pool.returns(job);
});
...
class GLoader extends GObject {
    constructor() {
        super();
        this._frame = 0;
        this._onComplete = (obj) => {
            this._node.emit(MyEvent.COMPLETE, obj);
        };
        ...
        this._node.addChild(this._container);
        this._content = this.container.addComponent(MovieClip);
        this._content.sizeMode = Sprite.SizeMode.CUSTOM;
        this._content.trim = false;
        this._content.setPlaySettings();
        this._externalLinker = new CCNodeLinker(this.container);
        this.loaded = false;
    }
    onEnable() {
        if (this._url && this._urlDirty) {
            if (this.isFirstTimeVisible && this._node.active) {
                this._whenUrlDirty();
            }
        }
    }
    ...
    _whenUrlDirty() {
        ...
        this.loadContent();
        this.updateGear(7);
        this._urlDirty = false;
    }
    _initContent2(obj) {
        this._content2 = obj;
        this._content2.ccRenderer = this._ccRenderer;
        let renderers = this._contentRenderers;
        if (!!renderers) {
            for (let i = 0, il = renderers.length; i < il; i++)
                this._content2.addRenderer(renderers[i].comp, renderers[i].params);
        }
        this.container.addChild(this._content2.node);
        let ccRender = this._content2.ccRender;
        if (ccRender) {
            if (typeof ccRender.open == "function")
                ccRender.open(this._data);
        }
        if (this._itemDataDirty) {
            //// this._itemDataDirty = false;
            this._content2.data = this._data;
        }
        let content2 = this._content2;
        if (this.playing) {
            if (typeof content2.play == "function")
                content2.play(this._playTimes);
        }
        this.sourceWidth = content2.width;
        this.sourceHeight = content2.height;
    }
    dispose() {
    ...
    setPrefabLoadedFun(func) {
        this._prefabLoadedFun = func;
    }
    _checkPrefabLoadedFun() {
        if (!this._prefabLoadedFun)
            return;
        if (this._url.search(/^[Pp]refab:/) == 0) {
            const resKeeper = componentUtils.getOrAddComponent(this._content, InternalResKeeper);
            this._prefabLoadedFun(resKeeper.getRes());
        }
    }
    get icon() {
        return this._url;
    ...
    set texture(value) {
        ...
        this._content.setSpriteFrame(value);
    ...
    get componentTouch() {
        return this._content2Touch;
    }
    set componentTouch(val) {
        this._content2Touch = val;
    }
    get componentRender() {
        return this._content2Render;
    }
    set componentRender(val) {
        this._content2Render = val;
    }
    get externalNode() {
        return this._externalLinker.target;
    }
    loadContent() {
        this.clearContent();
        if (!this._url)
        ...
        if (this._url.startsWith("ui://")) {
            var contentItem = this._contentItem = UIPackage.getItemByURL(this._url);
            if (!!contentItem && (contentItem.type == PackageItemType.Image || contentItem.type == PackageItemType.MovieClip)) {
                contentItem.getAsset(this.onLoaded.bind(this));
            }
            else {
    ...
    set data(val) {
        this._data = val;
        if (!this._urlDirty && !!this._content2) {
            this._content2.data = val;
        }
        else {
            this._itemDataDirty = true;
        }
    }
    get data() {
        return this._data;
    ...
    set ccRenderer(val) {
        if (this._ccRenderer === val) {
            return;
        }
        this._ccRenderer = val;
        if (this._content2Render) {
            if (!!this._content2) {
                this._content2.ccRenderer = val;
            }
        }
        else {
            if (this._ccRender != void 0) {
                super.removeRenderer(this._ccRender);
                if (typeof this._ccRender.dispose == "function")
                    this._ccRender.dispose(false);
                this._ccRender.destroy();
            }
            if (val != void 0) {
                this._ccRender = super.addRenderer(val);
            }
            else {
                this._ccRender = void 0;
            }
        }
    }
    get ccRender() {
        ...
        if (this._content2Render) {
            ...
            return (_a = this._content2) === null || _a === void 0 ? void 0 : _a.ccRender;
        }
        else {
            return this._ccRender;
        }
    }
    addContentRenderer(val, params) {
        let renderers = this._contentRenderers;
        if (!renderers)
            this._contentRenderers = renderers = [];
        renderers.push({ comp: val, params });
        if (!!this._content2) {
            return this._content2.addRenderer(val, params);
        }
    }
    removeContentRenderers() {
        let renderers = this._contentRenderers;
        if (!!renderers && renderers.length) {
            while (renderers.length) {
                const render = renderers.shift();
                this.node.getComponentsInChildren(render.comp).forEach(com => com.destroy());
            }
            this._contentRenderers = renderers = [];
        }
        if (!!this._content2) {
            this._content2.ccRenderer = null;
        }
    }
    /** 设置触摸穿透(加载好component后设置) */
    setTouchPenetrate() {
        this.component.opaque = false;
        let nowParentNode = this.component.node.parent;
        if (nowParentNode !== this.node) {
            this.node.addChild(this.component.node);
            nowParentNode.active = false;
        }
        this._content2Touch = true;
    }
    play(times = -1) {
        this._playing = true;
        ...
        let content2 = this._content2;
        if (!!content2) {
            if (typeof content2.setCompleteHandler == "function")
                content2.setCompleteHandler(this._onComplete);
            if (typeof content2.play == "function")
                content2.play(this._playTimes);
        }
        this.updateGear(5);
    ...
    stop() {
        this._playing = false;
        let content2 = this._content2;
        if (content2 && (typeof content2.stop == "function")) {
            content2.stop();
        }
        this.updateGear(5);
    }
    loadFromPackage(itemURL) {
        ...
        if (this._contentItem) {
            const curId = this._contentItem.id;
            if (this._contentItem.type == PackageItemType.Component)
                jobSystem.addJob(() => {
                    var _a;
                    // jobSystem.addJob本身是异步的，要关注contentItem的变化，防止contentItem变了但新的资源没有加载好
                    if (!!this._contentItem && this._contentItem.id != curId) {
                        this._contentItem.getAsset(this.onLoaded.bind(this));
                        return;
                    }
                    if (!this.loaded)
                        this._loadFromPackage(itemURL);
                    else {
                        (_a = this.node) === null || _a === void 0 ? void 0 : _a.emit(MyEvent.LOADED);
                    }
                });
        ...
        if (this._contentItem) {
            ...
            if (this._contentItem.type == PackageItemType.Image) {
                if (!this._contentItem.asset) {
                    ...
                    this._content.setSpriteFrame(this._contentItem.asset);
                    this._content.fillMethod = this._fillMethod;
                    if (this._fillMethod)
                    ...
                    this._content.fillClockwise = this._fillClockwise;
                    this._content.fillAmount = this._fillAmount;
                    this.updateLayout();
                }
            }
    ...
    _createComponent() {
        if (!this._contentItem || this.loaded)
            return;
        const itemURL = this._url;
        if (!itemURL) {
            this.setErrorState();
        }
        var obj = UIPackage.createObjectFromURL(itemURL);
        if (!obj)
            ...
            this.updateLayout();
            this.loaded = true;
            this._node.emit(MyEvent.LOADED);
        }
    }
    loadExternal() {
        var _a, _b;
        ...
        // else
        //      (UIConfig.defaultBundle ?? resources).load(this._url + "/spriteFrame", Asset, callback);
        let callback = (err, asset) => {
            //因为是异步返回的，而这时候可能url已经被改变，所以不能直接用返回的结果
            if (this._url != url || !isValid(this._node))
                return;
            if (err) {
                console.warn(err);
            ...
            if (asset instanceof Prefab) {
                this.onExternalPrefabLoadSuccess(asset);
                setTimeout(() => {
                    this._checkPrefabLoadedFun();
                }, 0);
            }
            else if (asset instanceof SpriteFrame)
                this.onExternalSpriteLoadSuccess(asset);
            else if (asset instanceof Texture2D) {
                let sf = new SpriteFrame();
                sf.texture = asset;
                this.onExternalSpriteLoadSuccess(sf);
            }
            ...
            this.loaded = true;
            this.node.emit(MyEvent.LOADED);
        };
        if (url.search(/^[Pp]refab:/) == 0) {
            const matcher = url.match(/^[Pp]refab:(.*)$/);
            const path = matcher[1];
            ((_a = UIConfig.defaultBundle) !== null && _a !== void 0 ? _a : resources).load(path, Prefab, callback);
        }
        else if (url.startsWith("http://") || url.startsWith("https://") || url.startsWith("/")) {
            let extName = this._getQueryParam(url, 'extName');
            if (extName)
                assetManager.loadRemote(url, { ext: '.' + extName }, callback);
            else
                assetManager.loadRemote(url, callback);
        }
        else
            ((_b = UIConfig.defaultBundle) !== null && _b !== void 0 ? _b : resources).load(this._url + "/spriteFrame", Asset, callback);
    }
    _getQueryParam(url, name) {
        var _a;
        const reg = new RegExp(`(^|&)${name}=([^&]*)(&|$)`);
        const result = ((_a = url.split('?')[1]) !== null && _a !== void 0 ? _a : '').match(reg);
        if (result == void 0)
            return null;
        else
            return result[2];
    ...
    onExternalLoadSuccess(texture) {
        ...
        this._content.setSpriteFrame(texture);
        this._content.type = Sprite.Type.SIMPLE;
    ...
    onExternalSpriteLoadSuccess(texture) {
        const resKeeper = componentUtils.getOrAddComponent(this._content, InternalResKeeper);
        resKeeper.setRes(texture);
        // this._content.spriteFrame = texture;
        this._content.setSpriteFrame(texture);
        this._content.type = Sprite.Type.SIMPLE;
        this.sourceWidth = texture.width;
    ...
    onExternalPrefabLoadSuccess(prefab) {
        const resKeeper = componentUtils.getOrAddComponent(this._content, InternalResKeeper);
        resKeeper.setRes(prefab);
        const node = instantiate(prefab);
        this._externalLinker.setTarget(node);
        ...
        this.updateLayout();
        const widget = componentUtils.getOrAddComponent(node, Widget);
    ...
    updateLayout() {
        ...
        if (this._autoSize) {
            ...
            this.setSize(cw, ch);
            this._updatingLayout = false;
            this._container.getComponent(UITransform).setContentSize(this._width, this._height);
        ...
        if (this._content2) {
            let targetW = this._content2.width;
            let targetH = this._content2.height;
            if (this.relations.getHasDef(RelationType.Width)) {
                targetW = this.width;
            }
            if (this.relations.getHasDef(RelationType.Height)) {
                targetH = this.height;
            }
            this._content2.setSize(targetW, targetH);
        }
        this._container.getComponent(UITransform).setContentSize(cw, ch);
        if (this._content2) {
            this._content2.setPosition(pivotCorrectX + this._width * this.pivotX, pivotCorrectY - this._height * this.pivotY);
    ...
    clearContent() {
        ...
        if (this._content) {
            ...
            this._content.setSpriteFrame(null);
        }
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (this._fillMethod != 0) {
            ...
            this._fillAmount = buffer.readFloat();
        }
        if (this._url)
            this._urlDirty = true;
        // if (this._url)
...
class GLoader3D extends GObject {
    ...
    onChangeSpine() {
        ...
        let skinName = this._skinName || this._content.skeletonData.getRuntimeData().skins[0].name;
        // if (this._content["_skeleton"].skin != skin)
        //     this._content.setSkin(skin);
        if (this._content["_skeleton"].skin.name != skinName) {
            this._content.setSkin(skinName);
        }
    }
    onChangeDragonBones() {
    ...
    loadExternal() {
        ...
        if (this._url.startsWith("http://")
            ...
            ((_a = UIConfig.defaultBundle) !== null && _a !== void 0 ? _a : resources).load(this._url, sp.SkeletonData, this.onLoaded2.bind(this));
    }
    onLoaded2(err, asset) {
        //因为是异步返回的，而这时可能url已经被改变，所以不能直接用返回的结果
...
class GButton extends GComponent {
    constructor() {
        super();
        this._touchCheckFlag = false;
        this._soundSilence = false;
        this._node.name = "GButton";
        this._mode = ButtonMode.Common;
        this._title = "";
        this._icon = "";
        this._sound = UIConfig.buttonSound;
        this._disableSound = UIConfig.buttonDisableSound;
    ...
    initSelected(val) {
        if (this._buttonController)
            this._buttonController.setupFlag = true;
        this.selected = val;
        if (this._buttonController)
            this._buttonController.setupFlag = false;
    }
    get selected() {
        return this._selected;
    }
    ...
    constructExtension(buffer) {
        ...
        if (this._mode == ButtonMode.Common)
            this.setState(GButton.UP);
        this._node.on(MyEvent.TOUCH_BEGIN, this.onTouchBegin_1, this);
        this._node.on(MyEvent.TOUCH_END, this.onTouchEnd_1, this);
        this._node.on(MyEvent.ROLL_OVER, this.onRollOver_1, this);
        this._node.on(MyEvent.ROLL_OUT, this.onRollOut_1, this);
        this._node.on(MyEvent.CLICK, this.onClick_1, this);
    }
    setup_afterAdd(buffer, beginPos) {
        super.setup_afterAdd(buffer, beginPos);
        if (!buffer.seek(beginPos, 6))
    ...
    onTouchBegin_1(evt) {
        if (evt.button != EventMouse.BUTTON_LEFT)
            return;
        if (!!this._touchCheckFun && !this._touchCheckFun.call(this)) {
            this._touchCheckFlag = false;
            return evt.preventDefault();
        }
        this._touchCheckFlag = true;
        this._down = true;
        evt.captureTouch();
        if (this._mode == ButtonMode.Common) {
            if (this.grayed && this._buttonController && this._buttonController.hasPage(GButton.DISABLED))
    ...
    soundSilence(v) {
        this._soundSilence = v;
    }
    set thouchCheck(checkFun) {
        this._touchCheckFun = checkFun;
    }
    onClick_1() {
        ...
        if (!this._soundSilence && this._sound) {
            let isDisable = false;
            if (!!this._buttonController) {
                if (this._buttonController.selectedPage == GButton.DISABLED || this._buttonController.selectedPage == GButton.SELECTED_DISABLED) {
                    isDisable = true;
                }
            }
            if (isPreventDefault)
                isDisable = true;
            var pi = UIPackage.getItemByURL(isDisable ? this._disableSound : this._sound);
            if (pi) {
                var sound = pi.getAssetImmediately();
                if (sound)
        ...
        if (this._mode == ButtonMode.Check) {
            if (this._changeStateOnClick) {
                ...
                this._node.emit(MyEvent.STATUS_CHANGED, this);
            }
        }
...
var ArrayCollectionEventKind;
(function (ArrayCollectionEventKind) {
    ArrayCollectionEventKind[ArrayCollectionEventKind["RESET"] = 0] = "RESET";
    ArrayCollectionEventKind[ArrayCollectionEventKind["REFRESH"] = 1] = "REFRESH";
    ArrayCollectionEventKind[ArrayCollectionEventKind["ADD"] = 2] = "ADD";
    ArrayCollectionEventKind[ArrayCollectionEventKind["REMOVE"] = 3] = "REMOVE";
    ArrayCollectionEventKind[ArrayCollectionEventKind["UPDATE"] = 4] = "UPDATE";
    ArrayCollectionEventKind[ArrayCollectionEventKind["REPLACE"] = 5] = "REPLACE";
    ArrayCollectionEventKind[ArrayCollectionEventKind["ADD_MULT"] = 6] = "ADD_MULT";
    ArrayCollectionEventKind[ArrayCollectionEventKind["ADD_MULT_AND_SCROLL"] = 7] = "ADD_MULT_AND_SCROLL";
})(ArrayCollectionEventKind || (ArrayCollectionEventKind = {}));
class ArrayCollection extends EventTarget {
    ...
    set source(value) {
        if (!value)
            value = [];
        this._source = value;
        this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.RESET);
    }
    refresh() {
        this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.REFRESH);
    }
    get length() {
        return this._source.length;
    }
    addItem(item) {
        let index = this._source.length;
        this._source.push(item);
        this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.ADD, index, item);
    }
    ...
    getItemIndex(item) {
        for (let i = 0, il = this._source.length; i < il; i++) {
    ...
    itemUpdated(item) {
        ...
        if (index != -1) {
            this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.UPDATE, index, item);
        }
    }
    removeItem(item) {
        let index = this.getItemIndex(item);
    ...
    removeItemAt(index, length = 1) {
        if (index < 0 || index >= this._source.length)
            return;
        if (this._source.length > length) {
            let items = this._source.splice(index, length);
            this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.REMOVE, index, items);
            return items[0];
        }
    }
    removeAllItems() {
        ...
        this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.REMOVE, 0, items);
    }
    replaceItemAt(item, index) {
        if (index < 0 || index >= this._source.length)
            return;
        let oldItem = this._source.splice(index, 1, item)[0];
        this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.REPLACE, index, item, oldItem);
        return oldItem;
    }
    replaceAll(newSource) {
        ...
        if (newLength < oldLength) {
            let items = this._source.splice(newLength, oldLength - newLength);
            this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.REMOVE, newLength, items);
        }
    ...
    addItems(items, scrollBottom = false) {
        let index = this._source.length;
        this._source.push(...items);
        if (scrollBottom) {
            this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.ADD_MULT_AND_SCROLL, index, items);
        }
        else {
            this.emit(MyEvent.COLLECTION_CHANGED, ArrayCollectionEventKind.ADD_MULT, index, items);
        }
    }
}
...
var ListPageMode;
(function (ListPageMode) {
    /** 垂直 */
    ListPageMode[ListPageMode["Vertical"] = 0] = "Vertical";
    /** 水平 */
    ListPageMode[ListPageMode["Horizontal"] = 1] = "Horizontal";
})(ListPageMode || (ListPageMode = {}));
class ListPageManager {
    get curVersion() { return this._curVersion; }
    constructor(list) {
        /** 当前的最高页数 */
        this.heapPage = 0;
        this._curVersion = 0;
    ...
    setOption(option) {
        ...
        list.node.off(MyEvent.SCROLL, this._onScroll, this);
        ++this._curVersion;
        this.option = option;
        this.heapPage = (_a = option.curHeapPage) !== null && _a !== void 0 ? _a : 0;
        if (!this.list.listData)
            this.list.setListData([]);
        if (option)
            list.node.on(MyEvent.SCROLL, this._onScroll, this);
    }
    quickConfig(quickConfigOption) {
        const { mode, endThreshold, intervalMs, totalPage, curHeapPage, getNextData, getTotalPage, convert, sorter, } = quickConfigOption;
        const listCollection = this.list.setListData([]);
        let curVersion;
        const nextPage = (pageManager) => __awaiter$1(this, void 0, void 0, function* () {
            const heapPage = pageManager.heapPage;
            const newDatas = yield getNextData(heapPage);
            if (curVersion !== pageManager.curVersion)
                return;
            let datas;
            if (convert)
                datas = newDatas.map(v => convert(v));
            else
                datas = newDatas;
            if (sorter)
                datas.sort(sorter);
            (yield listCollection).addItems(datas);
            if (getTotalPage) {
                const newPage = yield getTotalPage();
                this.option.totalPage = newPage;
            }
        });
        this.setOption({
            mode,
    ...
    nextPage() {
        ...
        if (this.pending)
            return;
        const totalPage = (_a = this.option.totalPage) !== null && _a !== void 0 ? _a : 1;
        if (totalPage <= this.heapPage)
            return;
        ++this.heapPage;
        this.option.nextPage(this);
    }
    _onScroll() {
        var _a;
        ...
        if (now < this._nextTime)
            return;
        const { mode, endThreshold, intervalMs, } = this.option;
        this._nextTime = (_a = now + intervalMs) !== null && _a !== void 0 ? _a : 500;
        const scrollPane = this.list.scrollPane;
        const lastPos = this._lastPos;
        const _endThreshold = endThreshold !== null && endThreshold !== void 0 ? endThreshold : 100;
        if (mode == ListPageMode.Vertical) {
            const curPos = this._lastPos = scrollPane.scrollingPosY;
            if (lastPos > curPos)
                //往下拉，即反方向拉的时候
                return;
            const viewMax = curPos + scrollPane.viewHeight;
            if (scrollPane.contentHeight - viewMax < _endThreshold)
                this.nextPage();
        }
        else {
            const curPos = this._lastPos = scrollPane.scrollingPosX;
            if (lastPos > curPos)
                return;
            const viewMax = curPos + scrollPane.viewWidth;
            if (scrollPane.contentWidth - viewMax < _endThreshold)
                this.nextPage();
        }
    }
}
...
class GList extends GComponent {
    constructor() {
        ...
        this._lineGap = 0;
        this._columnGap = 0;
        this._pageManager = new ListPageManager(this);
        this._lastSelectedIndex = 0;
        this._poolDetached = false;
        this._numItems = 0;
    ...
    set ccItemRenderer(val) {
        this._ccItemRenderer = val;
        var _child, _children = this._children;
        for (var i = 0, il = _children.length; i < il; i++) {
            _child = _children[i];
            _child.ccRenderer = val;
        }
    }
    get ccItemRenderer() {
        return this._ccItemRenderer;
    }
    get lineCount() {
    ...
    get isItemAsync() {
        ...
        if (!userData)
            return false;
        const value = userData.itemAsync || userData.ItemAsync;
        return value == "true" || value == "True";
    }
    get pageManager() {
        return this._pageManager;
    }
    onItemClick(listener, target) {
        this._node.on(MyEvent.CLICK_ITEM, listener, target);
    }
    onItemSelect(listener, target) {
        this._node.on(MyEvent.SELECT_ITEM, listener, target);
    }
    get itemPool() {
        return this._pool;
    }
    /**
     * 设置GList的对象池，需要在setData之前进行设置
     */
    set itemPool(value) {
        this._pool = value;
    }
    get poolDetached() {
        return this._poolDetached;
    }
    /**
     * 是否分离对象池，分离的对象池在GList.dispose的时候不会清理元素
     */
    set poolDetached(value) {
        this._poolDetached = value;
    }
    getFromPool(url) {
        if (!url)
            url = this._defaultItem;
        var obj = this._pool.getObject(url, this.isItemAsync);
        if (obj)
            obj.visible = true;
        return obj;
    }
    ...
    addChildAt(child, index) {
        ...
        child.on(MyEvent.CLICK, this.onClickItem, this);
        return child;
    }
    addItem(url) {
        if (!url)
        ...
        obj.ccRenderer = this.ccItemRenderer;
    ...
    addItemFromPoolAt(index, url) {
        var obj = this.getFromPool(url);
        obj.ccRenderer = this.ccItemRenderer;
        return this.addChildAt(obj, index);
    }
    removeChildAt(index, dispose) {
        var child = super.removeChildAt(index, dispose);
        if (!dispose)
            child.off(MyEvent.CLICK, this.onClickItem, this);
        return child;
    }
    ...
    get selectedIndex() {
        ...
        if (this._virtual) {
            for (i = 0; i < this._realNumItems; i++) {
                ...
                if (!!ii.obj && ii.obj.selected || !ii.obj && ii.selected) {
                    if (this._loop)
    ...
    onClickItem(evt) {
        ...
        this.dispatchSelectEvent(item, evt);
    ...
    dispatchSelectEvent(item, evt) {
        this._node.emit(MyEvent.SELECT_ITEM, item, evt);
    }
    dispatchItemEvent(item, evt) {
        this._node.emit(MyEvent.CLICK_ITEM, item, evt);
    }
    dispatchSelected() {
        let listDataNum = this._listData.length;
        if (this.selectedIndex >= 0 && this.selectedIndex < listDataNum) {
            let child = this.getChildAt(this.itemIndexToChildIndex(this.selectedIndex));
            let evt = borrowEvent(MyEvent.CLICK_ITEM, true);
            if (!!child)
                this._node.emit(MyEvent.CLICK_ITEM, child, evt);
        }
    }
    setSelectionOnEvent(item, evt) {
        if (!item || this._selectionMode == ListSelectionMode.None)
            return;
    ...
    /**
     * 使用 setListDataAndResizeToFit 替换
     * @deprecated
     */
    resizeToFit(itemCount = Number.POSITIVE_INFINITY, minSize = 0, maxSize = Number.POSITIVE_INFINITY) {
        if (this._isSetting) {
            this._setDataWaitFunc = () => {
                this._resizeToFit(itemCount, minSize, maxSize);
            };
        }
        else {
            this._resizeToFit(itemCount, minSize, maxSize);
    ...
    _resizeToFit(itemCount = Number.POSITIVE_INFINITY, minSize = 0, maxSize = Number.POSITIVE_INFINITY, offSize = 0) {
        this.ensureBoundsCorrect();
        ...
        if (this._virtual) {
            ...
            if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal)
                this.viewHeight = Math.min(lineCount * this._itemSize.height + Math.max(0, lineCount - 1) * this._lineGap + offSize, maxSize);
            else
                this.viewWidth = Math.min(lineCount * this._itemSize.width + Math.max(0, lineCount - 1) * this._columnGap + offSize, maxSize);
        }
        else if (itemCount == 0) {
            if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal)
    ...
    /**
     * 设置尺寸
     * 在列表赋值前，需要预设列表尺寸时使用 (固定item尺寸的列表有效)
     * @param itemCount
     * @param offSize 偏移值
     */
    preSetSizeToFit(itemCount, offSize = 0) {
        let obj = UIPackage.createObjectFromURL(this.defaultItem);
        let w = obj.width;
        let h = obj.height;
        obj.dispose();
        obj = null;
        const itemSize = new Vec2(w, h);
        if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.SingleRow)
            ...
            if (this._columnCount > 0)
                ...
                this._curLineItemCount = Math.floor((this._scrollPane.viewWidth + this._columnGap) / (itemSize.x + this._columnGap));
            ...
            if (this._lineCount > 0)
                ...
                this._curLineItemCount = Math.floor((this._scrollPane.viewHeight + this._lineGap) / (itemSize.y + this._lineGap));
                if (this._curLineItemCount <= 0)
                    this._curLineItemCount = 1;
            }
        }
        var lineCount = Math.ceil(itemCount / this._curLineItemCount);
        if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal)
            this.viewHeight = lineCount * itemSize.y + Math.max(0, lineCount - 1) * this._lineGap + offSize;
        else
            this.viewWidth = lineCount * itemSize.x + Math.max(0, lineCount - 1) * this._columnGap + offSize;
    }
    getMaxItemWidth() {
        var cnt = this._children.length;
        var max = 0;
    ...
    get selectedItemData() {
        let listData = this._listData.source, listDataNum = this._listData.length;
        switch (this._selectionMode) {
            case ListSelectionMode.Single:
                let selectedIndex = this.selectedIndex;
                if (selectedIndex >= 0 && selectedIndex < listDataNum)
    ...
    get selectedItemDatas() {
        let listData = this._listData.source;
        return this.getSelection([]).map((selectedIndex) => listData[selectedIndex]);
    }
    handleSizeChanged() {
        super.handleSizeChanged();
        this.setBoundsChangedFlag();
    ...
    _setVirtual(loop) {
        if (!this._virtual) {
            ...
            this._node.on(MyEvent.SCROLL, this.__scrolled, this);
            this.setVirtualListChangedFlag(true);
        }
    }
    /// <summary>
    ...
    handleScroll(forceUpdate) {
        ...
        if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal) {
            ...
            (_a = this._partner) === null || _a === void 0 ? void 0 : _a.scheduleOnce(() => { this.handleScroll1(true); }, 0);
            ...
            (_b = this._partner) === null || _b === void 0 ? void 0 : _b.scheduleOnce(() => { this.handleScroll2(true); }, 0);
    ...
    handleScroll1(forceUpdate) {
        ...
        var downward = oldFirstIndex > newFirstIndex;
        var childCount = this.numChildren;
        //上一次的最后一个idx
        var lastIndex = oldFirstIndex + childCount - 1;
        //重用的idx，如果是向下拉，就从最后的idx开始重用，否则从之前的第一个开始
        var reuseIndex = downward ? lastIndex : oldFirstIndex;
        var curX = 0, curY = pos;
        var needRender;
        var deltaSize = 0;
        ...
        while (curIndex < this._realNumItems && (end || curY < max)) {
            ...
            if (!ii.obj) {
                ...
                if (ii.obj) {
                    this.setChildIndex(ii.obj, downward ? curIndex - newFirstIndex : this.numChildren);
                }
                else {
                    ii.obj = this.getFromPool(url);
                    if (!ii.obj)
                        break;
                    ii.obj.ccRenderer = this.ccItemRenderer;
                    if (downward)
                ...
                needRender = forceUpdate && curIndex >= this._refreshIndex;
            if (needRender) {
                if (this._autoResizeItem && (this._layout == ListLayoutType.SingleColumn || this._columnCount > 0))
                    ii.obj.setSize(partSize, ii.obj.height, true);
                if (curIndex < this._listData.length) {
                    ii.obj.data = this._listData.source[curIndex];
                }
                if (this.itemRenderer)
                    this.itemRenderer(curIndex % this._numItems, ii.obj);
                if (ii.obj && curIndex % this._curLineItemCount == 0) {
            ...
            if (!downward && curIndex == newFirstIndex) //往上拉的时候要显示多一条才不会穿帮
                max += ii.height;
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (ii && ii.updateFlag != this.itemInfoVer && ii.obj) {
                if (!!ii.obj)
                    ii.selected = ii.obj.selected;
                this.removeChildToPool(ii.obj);
                ii.obj = null;
    ...
    handleScroll2(forceUpdate) {
        ...
        while (curIndex < this._realNumItems && (end || curX < max)) {
            ...
            if (needRender) {
                ...
                if (this._loop) {
                    ii.obj.data = this._listData.source[curIndex % this._numItems];
                }
                else {
                    if (curIndex < this._listData.length)
                        ii.obj.data = this._listData.source[curIndex];
            ...
            ii.updateFlag = this.itemInfoVer;
            ii.obj.setPosition(curX, curY);
            if (!forward && curIndex == newFirstIndex) //往后拉的时候要显示多一条才不会穿帮
                max += ii.width;
            curY += ii.height + this._lineGap;
            if (curIndex % this._curLineItemCount == this._curLineItemCount - 1) {
                curY = 0;
    ...
    handleScroll3(forceUpdate) {
        ...
        for (i = startIndex; i < lastIndex; i++) {
            if (i >= this._realNumItems)
                continue;
            ii = this._virtualItems[i];
            if (ii && ii.updateFlag != this.itemInfoVer)
                continue;
            if (!ii.obj) {
                //寻找看有没有可重用的
                ...
                needRender = forceUpdate && i >= this._refreshIndex;
                insertIndex = -1;
                lastObj = ii.obj;
            }
            if (needRender) {
                ...
                if (i < this._listData.length)
                    ii.obj.data = this._listData.source[i];
                if (this.itemRenderer != null)
    ...
    get listData() {
        return this._listData && this._listData.source;
    }
    set listData(val) {
    ...
    setListData(val) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._isSetting && this._reject) {
                yield this._p.then(() => {
                    this.setListData(val);
                });
                return;
            ...
            this._setDataWaitFunc = null;
            let _resolve;
            this._p = new Promise((resolve, reject) => {
                this._resolve = _resolve = resolve;
                this._reject = reject;
            });
            let dataProvider;
            let doFinish = () => {
                dataProvider = this.setListDataImmediately(val);
                if (this._setDataWaitFunc)
                    this._setDataWaitFunc.apply(this);
                this._isSetting = false;
                _resolve(dataProvider);
            };
            let doChildFinish = () => __awaiter(this, void 0, void 0, function* () {
                dataProvider = yield inerlistRenderHelper(this, val);
                if (this._setDataWaitFunc)
                    this._setDataWaitFunc.apply(this);
                this._isSetting = false;
                _resolve(dataProvider);
            });
            if (UIConfig.listSetDataCheckFlag) {
                if (!this.getDataProvider() && val.length) {
                    UIConfig.listSetDataCheck(this, doFinish, doChildFinish);
                }
                else {
                    doFinish();
                }
    ...
    setListDataImmediately(val) {
        val = val.concat();
        let dataProvider;
        this._numItems = val ? val.length : 0;
        dataProvider = this._listData;
        if (!dataProvider) {
            dataProvider = new ArrayCollection(val);
            dataProvider.on(MyEvent.COLLECTION_CHANGED, this._onCollectionChanged, this);
            this._listData = dataProvider;
        }
        else {
            dataProvider.source = val;
        }
        this._refreshIndex = 0;
        this.refreshList();
        return dataProvider;
    ...
    /**
     * 设置list数据，并且 ResizeToFit
     * @param val 列表值
     * @param minSize 根据列表类型 宽或高的最小尺寸
     * @param maxSize 根据列表类型 宽或高的最大尺寸
     */
    setListDataAndResizeToFit(val, minSize = 0, maxSize = 0, offSize = 0) {
        return this.setListData(val).then(() => {
            this._resizeToFit(val.length, minSize, maxSize, offSize);
        });
    }
    playChidTransitions(gapSec, transName) {
        if (this._virtual)
            this.checkVirtualList();
        var cnt = this._children.length;
        for (var i = 0; i < cnt; i++) {
            var obj = this._children[i];
            const transform = (!!transName) ? this.getTransition(transName) : obj.getTransitionAt(0);
            if (!!transform) {
                obj.alpha = 0;
                transform.startHook = (obj) => { obj.alpha = 1; };
                transform.play(undefined, 1, i * gapSec);
            }
        }
    }
    /**
     ...
     * @param maxSize 根据列表类型 宽或高的最大尺寸
     */
    setListDataAndResizeToFitImmediately(val, minSize = 0, maxSize = Number.POSITIVE_INFINITY, offSize = 0) {
        this.setListDataImmediately(val);
        this._resizeToFit(val.length, minSize, maxSize, offSize);
    }
    getOrSetDataProvider() {
        if (!this._listData) {
            this._numItems = 0;
            let dataProvider = new ArrayCollection([]);
            dataProvider.on(MyEvent.COLLECTION_CHANGED, this._onCollectionChanged, this);
            this._listData = dataProvider;
            dataProvider.source = [];
        }
    ...
    setDataProvider(val) {
        ...
        dataProvider = this._listData;
        if (dataProvider)
            dataProvider.off(MyEvent.COLLECTION_CHANGED, this._onCollectionChanged, this);
        dataProvider = this._listData = val;
        if (dataProvider)
            dataProvider.on(MyEvent.COLLECTION_CHANGED, this._onCollectionChanged, this);
        this.refreshList();
    }
    _onCollectionChanged(kind, index, item) {
        switch (kind) {
            case ArrayCollectionEventKind.ADD:
                this.itemAddedHandler(item, index);
                break;
            case ArrayCollectionEventKind.REMOVE:
                this.itemRemovedHandler(item, index);
                break;
            case ArrayCollectionEventKind.UPDATE:
            case ArrayCollectionEventKind.REPLACE:
                this.itemUpdatedHandler(item, index);
                break;
            case ArrayCollectionEventKind.RESET:
            case ArrayCollectionEventKind.REFRESH:
                this.refreshList();
                break;
            case ArrayCollectionEventKind.ADD_MULT:
                this.itemAddedMultHandler(item, index, false);
                break;
            case ArrayCollectionEventKind.ADD_MULT_AND_SCROLL:
                this.itemAddedMultHandler(item, index, true);
                break;
        }
    }
    refreshList() {
        ...
        if (this._virtual) {
            ...
            var obj;
            var listData = this._listData.source, listDataNum = this._listData.length, itemRenderer = this.itemRenderer;
            for (i = 0; i < value; i++) {
                obj = this._children[i];
                if (i < listDataNum)
                    obj.data = listData[i];
    ...
    itemAddedHandler(item, index) {
        this._numItems++;
        if (this._virtual) {
            this._realNumItems += this._loop ? 6 : 1;
            let ii, ii2;
            let i;
            let j;
            var oldCount = this._virtualItems.length;
            ...
            for (i = this._realNumItems - 2, j = i + 1; i >= index; i--, j--) {
                ii = this._virtualItems[i];
                ii2 = this._virtualItems[j];
                ii2.selected = ii.obj ? ii.obj.selected : ii.selected;
                if (ii2.obj)
                    ii2.obj.selected = ii2.selected;
            }
            ii = this._virtualItems[index];
            if (ii) {
                ii.selected = false;
                if (ii.obj)
                    ii.obj.selected = false;
            ...
            let obj;
            if (this.itemProvider == null)
                obj = this.addItemFromPoolAt(index);
            else
                obj = this.addItemFromPoolAt(index, this.itemProvider(index));
            setTimeout(() => {
                obj.data = item;
            }, 0);
        }
    }
    itemAddedMultHandler(items, index, scrollBottom) {
        for (let il = 0; il < items.length; il++) {
        ...
        if (scrollBottom) {
            this._scrollPane.scrollBottom(true);
        }
    }
    itemRemovedHandler(items, index) {
        let numChanged = items.length;
        this._numItems -= numChanged;
        if (this._virtual) {
            this._realNumItems -= this._loop ? 6 * numChanged : numChanged;
            let ii, ii2;
            let i, l;
            let j;
            for (i = index, j = index + numChanged, l = this._realNumItems; i < l; i++, j++) {
                ii = this._virtualItems[i];
                ii2 = this._virtualItems[j];
                if (!!ii) {
                    ii.selected = ii2 ? (ii2.obj ? ii2.obj.selected : ii2.selected) : false;
                    if (ii.obj)
                        ii.obj.selected = ii.selected;
                }
            }
            for (i = l, l = this._virtualItems.length; i < l; i++) {
                ii = this._virtualItems[i];
                if (!!ii) {
                    ii.selected = false;
                }
            ...
            this.removeChildrenToPool(index, index + items.length - 1);
        }
    }
    itemUpdatedHandler(item, index) {
        let childIndex = this.itemIndexToChildIndex(index);
        if (childIndex >= 0 && childIndex < this._children.length) {
            let obj = this.getChildAt(childIndex);
            obj.data = item;
        }
    }
}
...
class GComboBox extends GComponent {
    ...
    autoShowOrHideDropdown(values) {
        this._itemsUpdated = true;
        this._items = values;
        if (values && values.length && this.dropdown) {
            this._down = true;
            this._over = false;
            this.setState(GButton.DOWN);
            this.showDropdown();
    ...
    constructExtension(buffer) {
        ...
        if (str) {
            ...
            this._list.on(MyEvent.CLICK_ITEM, this.onClickItem, this);
            ...
            this.dropdown.on(MyEvent.UNDISPLAY, this.onPopupClosed, this);
        }
        this._node.on(MyEvent.TOUCH_BEGIN, this.onTouchBegin_1, this);
        this._node.on(MyEvent.TOUCH_END, this.onTouchEnd_1, this);
        this._node.on(MyEvent.ROLL_OVER, this.onRollOver_1, this);
...
class GSlider extends GComponent {
    ...
    constructExtension(buffer) {
        ...
        if (this._gripObject) {
            this._gripObject.on(MyEvent.TOUCH_BEGIN, this.onGripTouchBegin, this);
            this._gripObject.on(MyEvent.TOUCH_MOVE, this.onGripTouchMove, this);
        }
        this._node.on(MyEvent.TOUCH_BEGIN, this.onBarTouchBegin, this);
    }
    handleSizeChanged() {
        super.handleSizeChanged();
        if (this._barObjectH)
...
class GProgressBar extends GComponent {
    ...
    tweenLevelValue(value, maxValue, level, duration) {
        if (!this.node.activeInHierarchy || !this._initValueFlag) {
            this._max = maxValue;
            this.value = value;
            this._level = level;
            return;
        ...
        if (this._level == level) {
            this._max = maxValue;
            this._value = value;
            return GTween.to(oldValule, this._value, duration).setTarget(this, this.update).setEase(EaseType.SineOut);
        }
        else {
            if (this._level + 1 < level) { //跨2级以上先不处理！！TODO 
                this._max = maxValue;
                this._value = value;
                return GTween.to(oldValule, this._value, 0).setTarget(this, this.update).setEase(EaseType.SineOut);
            }
            else {
                const toMaxDuration = (this._max - this._value) / ((this._max - this._value) + value) * duration;
                GTween.to(oldValule, this._max, toMaxDuration).setTarget(this, this.update).setEase(EaseType.SineOut).onComplete(() => {
                    this._max = maxValue;
                    this._value = value;
                    return GTween.to(oldValule, this._value, duration - toMaxDuration).setTarget(this, this.update).setEase(EaseType.SineOut);
                });
            }
        }
        this._level = level;
    }
    reset(value = 50, maxValue = 100, level = 0) {
        this._max = maxValue;
        this.value = value;
        this._level = level;
        this._initValueFlag = false;
    }
    update(newValue) {
        ...
        if (this._titleObject) {
            switch (this._titleType) {
                ...
                case ProgressTitleType.Max:
                    ...
                    break;
                case ProgressTitleType.MaxTxt:
                    this._titleObject.text = "MAX";
                    break;
                case ProgressTitleType.None:
        ...
        if (!this._reverse) {
            ...
            if (this._barObjectH1) {
                if (!this.setFillAmount(this._barObjectH1, percent))
                    this._barObjectH1.width = Math.round(fullWidth * percent);
            }
            if (this._barObjectV) {
            ...
            if (this._barObjectH1) {
                if (!this.setFillAmount(this._barObjectH1, 1 - percent)) {
                    this._barObjectH1.width = Math.round(fullWidth * percent);
                    this._barObjectH1.x = this._barStartX + (fullWidth - this._barObjectH1.width);
                }
            }
            if (this._barObjectV) {
    ...
    constructExtension(buffer) {
        ...
        this._titleObject = this.getChild("title");
        this._barObjectH = this.getChild("bar");
        this._barObjectH1 = this.getChild("bar1");
        this._barObjectV = this.getChild("bar_v");
        ...
        if (this._barObjectH1) {
            this._barMaxWidth = this._barObjectH1.width;
            this._barMaxWidthDelta = this.width - this._barMaxWidth;
            this._barStartX = this._barObjectH1.x;
        }
        if (this._barObjectV) {
            this._barMaxHeight = this._barObjectV.height;
            this._barMaxHeightDelta = this.height - this._barMaxHeight;
...
class GScrollBar extends GComponent {
    ...
    constructExtension(buffer) {
        ...
        this._grip.on(MyEvent.TOUCH_BEGIN, this.onGripTouchDown, this);
        this._grip.on(MyEvent.TOUCH_MOVE, this.onGripTouchMove, this);
        this._grip.on(MyEvent.TOUCH_END, this.onGripTouchEnd, this);
        if (this._arrowButton1)
            this._arrowButton1.on(MyEvent.TOUCH_BEGIN, this.onClickArrow1, this);
        if (this._arrowButton2)
            this._arrowButton2.on(MyEvent.TOUCH_BEGIN, this.onClickArrow2, this);
        this.on(MyEvent.TOUCH_BEGIN, this.onBarTouchBegin, this);
    }
    onGripTouchDown(evt) {
        evt.propagationStopped = true;
        evt.captureTouch();
...
class GTreeNode {
    constructor(hasChild, resURL, render) {
        this._level = 0;
        this._resURL = resURL;
        this.ccItemRenderer = render;
        if (hasChild)
            this._children = new Array();
    }
    set ccItemRenderer(val) {
        this._ccItemRenderer = val;
        if (!!this._cell) {
            this._cell.ccRenderer = this._ccItemRenderer;
            if (!!this.data)
                this._cell.data = this.data;
        }
    }
    set data(v) {
        this._data = v;
        if (!!this._cell && !!this._cell.ccRenderer) {
            this._cell.data = v;
        }
    }
    get data() {
    ...
    set cell(v) {
        this._cell = v;
        if (this.ccItemRenderer) {
            this._cell.ccRenderer = this.ccItemRenderer;
            if (!!this.data)
                this._cell.data = this.data;
        }
    }
...
class GTree extends GList {
    ...
    createCell(node) {
        ...
        if (cc) {
            cc.on(MyEvent.STATUS_CHANGED, this.__expandedStateChanged, this);
        ...
        if (node.isFolder)
            node.cell.on(MyEvent.TOUCH_BEGIN, this.__cellMouseDown, this);
...
class PopupMenu {
    constructor(url) {
        ...
        this._contentPane.on(MyEvent.DISPLAY, this.onDisplay, this);
...
export { AlignType, ArrayCollection, AsyncOperation, AutoSizeType, BlendMode, ButtonMode, ByteBuffer, ChildrenRenderOrder, Controller, DragDropManager, EaseType, FillMethod, FillOrigin, FlipType, GButton, GComboBox, GComponent, GGraph, GGroup, GImage, GLabel, GList, GLoader, GLoader3D, GMovieClip, GObject, GObjectPool, GProgressBar, GRichTextField, GRoot, GScrollBar, GSlider, GTextField, GTextInput, GTree, GTreeNode, GTween, GTweener, GearAnimation, GearBase, GearColor, GearDisplay, GearDisplay2, GearFontSize, GearIcon, GearLook, GearSize, GearText, GearXY, GroupLayoutType, Image, JobSystem, ListLayoutType, ListPageManager, ListPageMode, ListSelectionMode, LoaderFillType, MovieClip, MyEvent, ObjectPropID, ObjectType, OverflowType, PackageItem, PackageItemState, PackageItemType, PackageLoadType, PopupDirection, PopupMenu, ProgressTitleType, RelationType, ScrollBarDisplayType, ScrollPane, ScrollType, Transition, TranslationHelper, UBBParser, UIConfig, UIObjectFactory, UIPackage, UIPackageLoadingMode, UserData, VertAlignType, Window, defaultParser, jobSystem, registerFont };
